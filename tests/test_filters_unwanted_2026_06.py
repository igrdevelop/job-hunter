...
def test_fullstack_backend_in_body_blocked() -> None:
    job = _job(
        title="FullStack Developer with Angular",
...
def test_clean_body_not_blocked() -> None:
    job = _job(title="Frontend Developer", body="Angular, TypeScript, RxJS, NgRx. Fully remote.")
...
def test_screen_flags_hybrid_city() -> None:
    reason = screen_job_text("Hybrid role, 3 days a week in our Warsaw office.")
    assert reason is not None
...
def test_screen_flags_fullstack_backend_with_title() -> None:
    reason = screen_job_text(
        "Backend in Java and Spring Boot.", title="Full Stack Developer with Angular"
    )
    assert reason is not None
...
def test_screen_now_warns_on_off_domain_title() -> None:
    ...
    reason = screen_job_text(
        "Great backend-free role building dashboards.", title="Senior Software Engineer"
    )
    assert reason is not None
