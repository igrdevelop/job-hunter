...
def test_mark_orphans_expired_stamps_and_clears(tracker_db):
    _insert_db_row(tracker_db, row_id="orph0001", url="https://example.com/1",
                   sent="", sheets_dirty=1, sheets_row=492)
...
def test_mark_orphans_expired_preserves_existing_sent(tracker_db):
    _insert_db_row(tracker_db, row_id="orph0002", url="https://example.com/2",
                   sent="28 05", sheets_row=10)
...
def test_reconcile_partial_read_guard(tracker_db):
    ...
    for i in range(10):
        _insert_db_row(tracker_db, row_id=f"row{i:05d}",
                       url=f"https://example.com/{i}", sent="")
    ...
    with get_db(tracker_db) as conn:
        n = conn.execute(
            "SELECT COUNT(*) c FROM applications WHERE sent='EXPIRED'"
        ).fetchone()["c"]
    assert n == 0
