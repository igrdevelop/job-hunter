...
### Очередь 3 — высокий объём, тяжёлый парсинг — ⛔ обе отложены после разведки
- **Wellfound** ⛔ — жёсткий 403, бот-защита непробиваема `requests`/`cloudscraper`;
  нужен Playwright + login (Docker-блокер)
- **Jobgether** ⛔ — нет чистого JSON-листинга, хрупкий Tailwind-DOM, серверного
  frontend-фильтра нет, 0 попаданий под title-фильтр в dev-категории

Разведка подтвердила исходную оценку «низкий ROI». Деталь см. `QUEUE-3-hard.md`.
