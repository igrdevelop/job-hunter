...
> **СТАТУС (2026-06-08):** Очередь 2 закрыта на **Built In ✅ + JustRemote ✅**.
> **Remote.co — ОТЛОЖЕН** (см. §2.3): активный anti-bot (throttle серии запросов,
> cloudscraper тоже блокируется), единственный путь — хрупкий DOM-скрейп 422KB
> страницы, которую невозможно даже провалидировать; источник дублирует уже
> покрытые WWR/Remotive и имел самый низкий ROI в очереди. Решение пользователя —
> отложить, не тащить непроверяемый парсер. Вернуться при явной потребности
> в объёме (тогда — низкочастотный cloudscraper + ручная валидация структуры).
...
## 2.3 Remote.co (`remote.co`) — ⛔ ОТЛОЖЕН (2026-06-08)
...
### Итог разведки (2026-06-08) — почему отложен
- `https://remote.co/remote-jobs/developer/` → 200, **422KB HTML**, без RSS,
  без подтверждённого `__NEXT_DATA__`/JSON-LD (проверить не успел — заблокировали).
- `https://remote.co/feed/` → 200, 60 `<item>`, но это, скорее всего, **блог**
  Remote.co, не вакансии. Категорийный job-feed `/remote-jobs/developer/feed/`
  → **404**.
- **Активный anti-bot:** после серии проб сайт начал сбрасывать соединения /
  таймаутить **даже под `cloudscraper`** (IP-throttle на всплеск запросов).
- Единственный реальный путь — хрупкий DOM-скрейп тяжёлой страницы, который
  невозможно было даже провалидировать в моменте.
- Источник **дублирует** уже покрытые WWR + Remotive и имел **самый низкий ROI**
  в очереди.

**Решение:** отложить (решение пользователя). Не тащить непроверяемый хрупкий
парсер ради low-value дубль-источника. Если вернёмся — низкочастотный
`cloudscraper` (1 запрос/цикл, чтобы не триггерить throttle) + ручная валидация
структуры листинга; рассмотреть, не покрывает ли его уже поток WWR/Remotive.

### Разведка (исходный план, если возвращаться)
