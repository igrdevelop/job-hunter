...
| 1.5 | **Market memory — remember every listing the hunt sees** | [MARKET_MEMORY_PLAN](MARKET_MEMORY_PLAN.md) | M0 tool + M1–M3 shipped 2026-09-13 (PR #279); M0.a run on prod 2026-09-14 — rules 1–3 PASS. Next: after ~2 weeks check `postings_seen` growth/day (revised M0.b) to settle TTL, then M4 `/market` digest after ~4 weeks of rows | M4 one PR once the data exists |
