...
| 1.8 | **Pipeline page — where every vacancy is right now** | [PIPELINE_VIZ_PLAN](PIPELINE_VIZ_PLAN.md) | M0 run on prod 2026-09-22 (#287): rule 1 PASS (metrics coverage 100%), rules 2/3/5 FAIL → M1 = `start`+refine events, `hunt_runs`, orphan-run stamping, plus `queued_at` (#288); rule 4 was a definition (owner-declined dash rows are not "ready"), fixed in the tool. Next: merge the four M1 PRs, then M2 contract + `GET /pipeline/snapshot` (job-hunter-api) and M3 `/pipeline` page (job-hunter-site) | M1: four small PRs; M2 + M3 one PR per repo |
