...
| 1.5 | **Market memory — remember every listing the hunt sees** | [MARKET_MEMORY_PLAN](MARKET_MEMORY_PLAN.md) | M0: `python tools/market_memory_m0.py` (read-only live probe: new-unique share, salary/location parse coverage, filter-verdict split) + the `source_runs` volume query on prod. Rule: < 30 new listings per sweep closes the plan; < 25% parseable salary drops the pay track. Then M1 `postings_seen` ($0, best-effort write in the hunt loop), M2 `skip_reason`, M4 `/market` digest (reports only — nothing feeds back into the hunt or the apply pipeline, owner decision). The X side of #276's `outcome_label` | M0 one evening; M1 one PR |
