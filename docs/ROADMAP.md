...
| 1.7 | **Apply failure queues — tell "our bug" from "bad vacancy"** | [APPLY_FAILURE_QUEUES_PLAN](APPLY_FAILURE_QUEUES_PLAN.md) | M0 tool shipped 2026-09-21 (`tools/fail_signatures.py`), not yet run on prod. Next: `python tools/fail_signatures.py --db tracker.db` on the deploy host. It counts the 2026-09-10 CLI-argv incident's damage and decides whether M2+M3 (classifier + `BLOCKED` queue) are worth building. M1 (post-deploy CLI canary) ships regardless | M0 on prod: minutes; M1 one PR; M2+M3 one PR each |
