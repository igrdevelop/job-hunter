...
| `live` | object \| null | The hunt running right now (pipeline control plan, 2026-09-25). `null` when the `hunt_live` table (created lazily by the first hunt) is missing or lacks a column. Otherwise `{active: row \| null, last: row \| null}`: `active` = the NEWEST row (`ORDER BY started_at DESC, rowid DESC LIMIT 1`) when its `finished_at IS NULL`, else `null`; `last` = the newest row with `finished_at IS NOT NULL` (same order). Not windowed. |
