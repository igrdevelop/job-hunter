...
генерация CV → рендер PDF → вердикт (92)
  ...
  → всё ещё < 95? → РАУНД 2 (эскалация): разрешено ДОБАВИТЬ технологии из
       вакансии, которых у кандидата нет — как обычные пункты в skills/summary,
       без страховочных пометок; если нужна привязка к опыту — вплести в один
       из гибких проектов Altoros (2018–2022). КАЖДАЯ такая добавка дописывается
       в to_learn (колонка To Learn в трекере) — владелец подучит перед
       собеседованием
...
2. **Политика раундов — эскалация:**
   ...
   - **Раунд 2 (stretch):** разрешено добавить технологии из СПИСКА ВАКАНСИИ
     (missing_keywords вердикта), отсутствующие в профиле — как ОБЫЧНЫЕ записи
     в skills и/или summary, без пометок «familiar»/«working knowledge»
     (решение владельца: если надо добавить — добавляем). Каждая такая добавка
     обязана попасть в `content["to_learn"]` (→ колонка To Learn в
     трекере/Sheets) — это учебный долг владельца перед собеседованием.
     **Куда вписывать:** по умолчанию — skills/summary. Если технология там
     «не вписывается» (вердикт прямо требует production-evidence, или skills-
     запись выглядит сиротой) — РАЗРЕШЕНО вплести её в ОДИН из клиентских
     проектов Altoros (2018–2022: E-commerce / Insurance / Healthcare /
     Grant Management — владелец считает их гибкими; прецедент уже есть:
     React-прототипы в e-commerce). Правила вставки: выбрать самый
     правдоподобный проект по эпохе и стеку (Vitest в Insurance на Angular 6–8
     образца 2018 — неправдоподобно; E-commerce — самый гибкий); добавка в
     Stack-line и/или один скромный буллет; БЕЗ выдуманных метрик/чисел.
     НЕПРИКАСАЕМЫЕ: Atruvia, Fairmarkit, Intel, SII, SolbegSoft — недавние
     и проверяемые работодатели, туда stretch-технологии не вписываются.
     Запрещены на любом раунде: выдуманные работодатели, проекты, метрики, годы.
...
4. **Judge остаётся enforcement-воротами:** после каждого rewrite контент
   повторно проходит скрабы + claim judge + языковой гейт. Замечание: stretch-
   добавки раунда 2 (и в skills, и в Altoros-вставках) judge НЕ считает
   fabrication — его правило «отсутствует и в профиле, И в вакансии», а эти
   технологии взяты из вакансии. Это штатное поведение, не дыра: judge ловит
   то, что цикл добавлять не разрешал (выдуманные метрики, клеймы про
   технологии, которых нет даже в вакансии, вставки в неприкасаемых
   работодателей ловятся тестом промпта, а не judge'ем).
...
def refine_loop(content, job_text, base_cv, folder, verdict, *,
    ...
    """The loop. Round N (1-based) gets its own rewrite policy:
      ...
      round 2 = STRETCH — may ADD posting technologies absent from the profile
                as PLAIN skills/summary entries (no "familiar"/"working
                knowledge" hedging — owner's decision); every such addition MUST
                also be appended to content["to_learn"] (comma-joined, deduped)
                so it reaches the tracker's To Learn column. Placement: default
                skills/summary; when experience evidence is needed, MAY be woven
                into ONE Altoros client project (2018–2022, era/stack-plausible,
                stack-line + one modest bullet, no invented numbers). NEVER into
                Atruvia/Fairmarkit/Intel/SII/SolbegSoft; never invented
                employers/projects/metrics/years — on any round.
    Per round:
...
8. **Эскалация:** раунд 1 промпт БЕЗ stretch-разрешения; раунд 2 промпт С ним
   (проверить содержимое промптов по маркерам: разрешение на Altoros-вставку
   присутствует, список неприкасаемых работодателей присутствует).
