...
генерация CV → рендер PDF → вердикт (92)
  ...
  → всё ещё < 95? → РАУНД 2 (эскалация): разрешено ДОБАВИТЬ технологии из
       ...
       без страховочных пометок. КАЖДАЯ такая добавка дописывается в to_learn
       (колонка To Learn в трекере) — владелец подучит перед собеседованием
...
2. **Политика раундов — эскалация:**
   ...
   - **Раунд 2 (stretch):** разрешено добавить технологии из СПИСКА ВАКАНСИИ
     ...
     ГРАНИЦА (на любом раунде): добавки живут в skills/summary как компетенции;
     запрещено вписывать их в experience-буллеты как использованные в конкретных
     проектах, и запрещены выдуманные работодатели, проекты, метрики, годы.
...
4. **Judge остаётся enforcement-воротами:** после каждого rewrite контент
   повторно проходит скрабы + claim judge + языковой гейт. Замечание: skills-
   добавки раунда 2 judge НЕ считает fabrication (его правило — «отсутствует и
   в профиле, И в вакансии», а эти технологии взяты из вакансии) — это штатное
   поведение, не дыра. Judge при этом ловит именно нарушение границы: если
   rewrite вплёл добавку в experience-буллет как выдуманное использование в
   проекте — такой клейм отсутствует и в профиле, и в вакансии → fabrication →
   авторемонт.
...
def refine_loop(content, job_text, base_cv, folder, verdict, *,
    ...
    """The loop. Round N (1-based) gets its own rewrite policy:
      ...
      round 2 = STRETCH — may ADD posting technologies absent from the profile
                ...
                so it reaches the tracker's To Learn column. Boundary: additions
                go to skills/summary only — NEVER woven into experience bullets
                as project usage; never invented employers/projects/metrics/
                years — on any round.
    Per round:
