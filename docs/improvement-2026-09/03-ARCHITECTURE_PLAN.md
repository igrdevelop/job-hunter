...
1. ~~Postgres на Stage 2, а не Stage 8?~~ **Да** — поправка П1 принята.
3. ~~CLI-фоллбэк только для владельца?~~ **Нет** — поправка П2 отклонена,
   глобальные профиль/shadow/фоллбэк остаются. Пересмотр привязан к первому
   платящему клиенту, не к первому внешнему пользователю.

Открыто:

2. Alembic в репо бота как единственный владелец DDL, NestJS перестаёт
   зеркалировать схему (да/нет)? — следующий по очереди, П1 без него неполна.
