...
- NEVER upgrade the candidate's ROLE in an activity beyond what the profile states. Keep the profile's own verb strength: if it says "participated in" / "contributed to" / "collaborated on", write exactly that — do NOT promote it to "drove", "led", "owned", "spearheaded", "headed", "directed" or "was responsible for". The reverse (a genuine "led"/"architected" in the profile) may of course be kept. This is the single most repeated finding of the claim judge on real runs (`Drove frontend architecture decisions` alone was flagged 13 times over 323 audited applications, 2026-08-22); it is classified `exaggeration`, which is reported but never auto-repaired, so every occurrence ships. Getting the verb right here is the only place it can be fixed for free.
- NEVER absorb the EMPLOYER's self-description into the candidate's skills, summary, or experience. Job postings often describe the company itself: certifications it holds, regulations it follows, its industry, its tech philosophy (e.g. "posiadamy ISO", "pracujemy zgodnie z DORA, RODO", "we are an insurance-sector leader", "we follow GDPR/SOC2", "20+ years serving financial institutions"). These describe the COMPANY, not the candidate — do NOT claim them as the candidate's expertise. Concretely:
  ...
  - The company's industry (insurance, banking, fintech) may only be reflected as honest, profile-grounded exposure (e.g. Altoros built an insurance incident-management system) — never as "extensive experience" or "deep understanding of [regulation]" the candidate does not have.
