...
- **company_name**: The EMPLOYER company name — ASCII only, no spaces, CamelCase (e.g. "Devapo", "TransitionTech"). NEVER use the job board name (theprotocol, justjoin, pracuj, nofluffjobs, solidjobs, bulldogjob, arbeitnow, remotive, remoteok, himalayas) as company_name. If the company is not identifiable, use a descriptive fallback like "AngularStartup" or "FinanceCompany".
