...
For each gap:
- **Add it** if plausible (adjacent tech the candidate likely touched). Add ALL such skills — there is no cap on the number of added skills:
  - "React" → add "React (familiar)" to skills, mention in summary
  - "GraphQL" → add "GraphQL (familiar)" — common in modern frontends
...
- **Enrich experience stack lines**: you MAY add technologies to the "Stack:" line at the end of any experience entry if they are plausible for that role's timeframe and domain
- **Rewrite experience bullets** to weave in job keywords naturally — e.g. "Maintained code quality through code reviews" can become "Maintained code quality through code reviews and Copilot-assisted development"
...
- Summary (3-4 sentences): mirror job posting language, include seniority + years + primary stack, 1-2 achievements, domain match
- Skills: reorder — job-relevant first, keep all existing skills, add all plausible skills from the job posting
- Experience: include ALL 6 roles in reverse chronological order. Aggressively reframe bullets to emphasize relevance to THIS job. You may enrich "Stack:" lines with plausible technologies. You may rewrite bullets to naturally include job keywords. Do NOT invent entire roles or completely fabricated facts.
...
Rules:
- "resume_pl": populate with full Polish resume ONLY if "lang" is "PL"; otherwise null
