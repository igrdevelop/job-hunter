"""
tools/repair_tracker.py — Reconcile tracker.xlsx with Applications/ folder.

What it does:
  1. Backs up tracker.xlsx before any changes
  2. Reads all existing tracker URLs (dedup key)
  3. Scans Applications/{date}/{company}/content.json — adds missing rows
  4. Removes incomplete rows (no date AND no company AND no ATS)
  5. Sorts all data rows by Date ascending
  6. Saves tracker.xlsx
