"""
hunter/apply_cli.py — CLI-mode apply pipeline (Claude Pro subscription).

Runs `claude -p --dangerously-skip-permissions /apply <url>` as a subprocess,
then post-processes the folder it creates (cover letter review, React-only check).
"""
...
def _find_new_folder(before: set[str], timeout: int = 300) -> str | None:
    """Detect a newly created application folder; returns relative path or None."""
...
def main_cli(url: str, force: bool = False, full: bool = False) -> None:
    ...
    # Pre-fetch job text so Claude doesn't have to WebFetch
    apply_input = url
    ...
    # Expired check before spinning up Claude CLI
    if job_text:
    ...
    if new_folder:
        ...
        if content_json_path.exists():
            try:
                ...
                if "react" in _cli_stack and "angular" not in _cli_stack and not force:
                    notify(
