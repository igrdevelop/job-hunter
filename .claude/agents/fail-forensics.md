...
| LLM call | timeout, exit 124-ish, or the subprocess killed at `APPLY_AGENT_TIMEOUT_SEC` | If the run may have gone through the Claude CLI, the effective cap is `APPLY_AGENT_CLI_TIMEOUT_SEC` (2700s) — a CLI-served vacancy spawns 10–20 sequential calls |
