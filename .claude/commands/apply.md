...
**Add it** if it's plausible — adjacent tech Ihar has almost certainly touched. Add ALL such plausible skills — there is no cap:
- e.g. job says "React" → Ihar knows Angular deeply; add "React" to skills, mention "familiar with React ecosystem" in summary
- e.g. job says "GraphQL" → common in modern frontends; add to skills as "GraphQL (familiar)"
- e.g. job says "Docker" → DevOps adjacent, he worked with Jenkins/CI/CD; add "Docker (basic)"
- e.g. job says "Storybook" → UI component tool, natural for any Angular/React dev
- e.g. job says "GitHub Copilot" / "AI tools" → plausible for any modern developer; add "GitHub Copilot"
- e.g. job says "Playwright" → natural given Cypress/Jest experience; add "Playwright"

**Reframe existing experience** if possible:
- e.g. job says "micro-frontends" → Ihar used Nx monorepo, which is the same concept; rephrase bullet to include the term
- e.g. job says "web performance optimization" → he did this; add explicit keyword to bullet

**Enrich experience stack lines**: you MAY add technologies to the "Stack:" line of any experience entry if they are plausible for that role's timeframe and domain.

**Rewrite experience bullets** to weave in job keywords naturally:
- e.g. "Maintained code quality through code reviews" → "Maintained code quality through code reviews and Copilot-assisted development"
- e.g. "Built features in Angular 19" → "Built features in Angular 21, leveraging the latest Signals API"
...
- Seniority + years + primary stack matching the job
...
**Work Experience** — keep all roles, aggressively reframe bullets to emphasize relevance to THIS job. You may enrich "Stack:" lines with plausible technologies. You may rewrite bullets to naturally include job keywords. Do NOT invent entire roles or completely fabricated facts.
...
Rules for the JSON:
- `resume_pl`: populate with full Polish-translated resume data ONLY if `lang` is `PL`, otherwise set to `null`
