...
Extract:
- **Company name** (ASCII only, no spaces, CamelCase, e.g. "Devapo") — NEVER use the job board name (theprotocol, justjoin, pracuj, nofluffjobs, solidjobs, arbeitnow, remotive, remoteok) as company name. Use the actual employer. If not identifiable, use a descriptive fallback like "AngularStartup".
- **Job title** (exact)
