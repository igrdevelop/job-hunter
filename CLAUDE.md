...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-15 | opus | **Sheets `Folder` column shows `<date>/<Company>` instead of the absolute container path** (owner: "очень длинная спецификация, мне бы хватило числа и имени папки" — every row read `/app/users/d6c0ca67-…-aa40a6307e00/Applications/2026-08-14/CoreView`, ~60 chars of which only the last two segments say anything). The shortening lives at the Sheets boundary ONLY: `gsheets_client.short_folder()` drops everything up to and including the `Applications` component (falls back to the last two segments when there is no such component, returns unparseable input unchanged, handles Windows separators), applied inside `_row_to_list` — the single choke point every write path goes through (`append_rows` / `update_row` / `batch_write_all`), so `mirror_new_row`, `resync_dirty`, `push_missing_rows` and the bootstrap all get it without touching `gsheets_sync` at all. tracker.db deliberately keeps the FULL path: `gdrive_sync.upload_missing_folders`, the re-post gate's donor lookup and every local file read resolve it. Known asymmetry, accepted: `tracker.insert_pulled_rows` (the fresh-DB dedup self-heal) will now insert the SHORT value for a row that exists only in the Sheet — such a row has no local folder to resolve anyway, and gdrive's backfill already skips a non-resolving path as `skipped_missing` (debug log, no error). No pull/conflict path compares Folder, so the write-only transformation cannot dirty a row. Rows already mirrored keep their long value: a backfill tool was written and then dropped from the change on the owner's call — "на старые по фигу, новые только меня будут интересовать". 10 new tests in `test_gsheets_client.py`; the pre-existing `test_roundtrip` now documents Folder as the one column that does NOT round-trip. Full suite green (2605 with the dropped tool's 4 tests), ruff clean. |
