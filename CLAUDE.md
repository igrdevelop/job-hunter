...
hunter/
  ...
  metrics.py                Per-run metrics log for the apply pipeline (docs/
                            ...
                            test_metrics_refine_events.py pins the refine rows. STILL
                            OPEN from the M0 gap list: a run killed by the worker's
                            timeout never gets `finish_run` (leaks `finished_at IS
                            NULL`), and there is no prune. Read side today:
                            `count_runs_since()` for a `/status` line, and
