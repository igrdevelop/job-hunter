...
through the paste flow (no real LinkedIn post permalink exists — see below) in BOTH
the AUTO_APPLY and manual-card code paths, using the saved post text automatically —
no manual re-paste needed either way.
...
`LINKEDIN_SCOUT_RELAY_ENABLED` (default true) and in the fetch-dispatch roster. It is
NOT `manual_only` (new `BaseSource.manual_only: bool = False` attribute, added for any
future source that DOES want to force a card — `hunter/main.py`'s ACT step partitions
`new_jobs` on it before the `AUTO_APPLY` branch, currently a no-op since nothing sets it
True). Paste-flow wiring exists on BOTH code paths since either could run depending on
