...
| 2026-09-12 | fable | **VPS disk full again two weeks after the `until=168h` prune — image retention is now a bounded COUNT, and the free-space floor is measured, not guessed.** On 2026-09-12 the 75 GB disk hit 100% with 17 images / 73 GB, 63 GB reclaimable: TEN `ghcr.io/igrdevelop/job-hunter:<sha>` images of ~8.1 GB each (Playwright chromium + LibreOffice + Claude CLI — the `docker images` SIZE on the host), all younger than two days. The 2026-08-29 fix capped an image's AGE, not the number of images, so a busy merge day added tens of GB in hours that neither the deploy-time `docker image prune -a -f --filter until=168h` nor the identical host cron could touch — both ran, both freed nothing. Second defect in the same block: the pre-pull free-space gate used a 5000 MB floor written against a "~2 GB compressed" figure that was never the extracted size, so a deploy passed the check and filled the disk mid-extraction; a side effect of that disk-full start was the bot's own startup token refresh (`hunter/oauth_alert.py::refresh_or_alert` → `token_file.write_text`) truncating `gsheets_token.json` to 0 bytes — the open succeeded, the write hit ENOSPC — which killed the Sheets mirror until the token was restored by hand. **Fix:** new `scripts/docker_prune.sh` (POSIX sh, host-script convention of `offhost_backup.sh`, but unlike that one it IS run by CI) keeps the image behind the live `job-hunter` container plus `KEEP_PREVIOUS=1` newest other job-hunter images as a rollback target, `docker rmi -f`s the rest (images used by ANY running container skipped, so a manually started rollback container is safe; everything removed is re-pullable by SHA), then still runs the generic `until=168h -a` prune for the sibling projects' leftovers on the shared daemon (job-hunter-api/arifma/psybook — that side effect stays intended). `deploy.yml` curls it at the deployed commit SHA next to `docker-compose.yml` and runs it BEFORE `docker compose pull` (reclaim) and AFTER `up -d` (settle), so the steady state between deploys is exactly two job-hunter images (~16 GB), peaking at three during a deploy; the host cron line in docs/DEPLOY.md "Disk hygiene" now runs the same local copy (`DOCKER=/usr/bin/docker`, cron's minimal PATH) instead of its own `prune` command, so the two cannot drift. Floor raised 5000 → 12000 MB (8.1 GB real + ~50% headroom for the compressed layers on disk during extraction), the stale comment corrected, and the DEPLOY.md server reference gains `docker images ghcr.io/igrdevelop/job-hunter` as the number to re-measure whenever the Dockerfile grows. `set -eo pipefail` and the Telegram failure step are untouched. **Verified without a docker binary:** the script was driven against a stub `docker` in five scenarios — keep=1 with a manually rolled-back live container and a second container pinning a third image (only the two oldest removed), keep=0, no container at all (newest kept), `rmi` refusing (warns, exit 0, generic prune still runs), and `docker images` itself failing (exit 1, nothing removed) — the last of which caught a real defect in the first cut: POSIX sh has no `pipefail`, so `docker images | awk` turned a dead daemon into a silent "nothing to remove"; each docker listing is now captured on its own line. `bash -n` on the script and on the deploy script extracted from the workflow YAML, `yaml.safe_load` on the workflow. **Not done here, on purpose:** the VPS itself (the one-off cleanup and installing the new cron line are owner-run on live prod), and hardening `refresh_or_alert`'s token write to temp-file + `os.replace` so a full disk can never leave a 0-byte token again — flagged as its own follow-up rather than folded into a deploy-config change. |
| 2026-09-12 | opus | **Outcome round-trips through Sheet column O.** #276 put `outcome_label` in tracker.db and `/outcome` in Telegram, but the owner's daily workflow is the Sheet, which could neither show nor edit it: the dirty-row push writes only A–K, so `set_outcome`'s `sheets_dirty=1` never carried the label anywhere, and the pull parsed rows by `enumerate(COLUMNS)`, blind past K. Added `hunter/outcome_writer.py` as the fifth non-overlapping writer rather than extending `COLUMNS` (which would make every A–K push blank an owner-typed label). Push: `resync_dirty` writes O for each dirty row that carries a label, at the row the A–K write actually landed on, and a failed O write keeps the row dirty; `/outcome` and its button push the cell at once (`gsheets_sync.mirror_outcome`, which alone writes a blank, for `clear`). Pull: `read_all` reads A:O, `_merge_outcomes` lets a valid label win over an empty or different DB value, ignores a garbage cell with a warning, and never lets a blank cell clear the DB — rows labelled before this shipped, or whose O write was lost, show a blank that is not a deletion. **One guard the brief did not name:** a dirty DB row keeps its value against the cell, because it holds an `/outcome` change the Sheet has not received yet — without it a failed immediate write plus a pull landing before the 5-min resync would silently revert the Telegram press to the stale cell. Known gap, shared with the A–K push: an owner edit in O on a row that turns dirty for another reason before the next pull gets overwritten on resync; a per-column dirty flag would close it and was not worth a migration at this volume. O also gets its header and a strict dropdown of `OUTCOME_LABELS` (`setDataValidation`, once per process, non-fatal). Not done: no bulk backfill tool like `tools/sync_verdicts.py` — rows re-appended by `/gsheets_push_missing` show O blank until their next `/outcome` or resync. The pre-PR code review caught one more: the immediate write used the cached `sheets_row` blind, and rows deleted above it in the Sheet would put the label next to a DIFFERENT application, which the next pull then copies into that application's DB row — `mirror_outcome` now reads K{row} first and skips on an ID mismatch (the resync does not need this; it rewrites the whole A–K row at that position). `tests/test_outcome_sheet_sync.py` (22 tests); mutation-verified: the dirty guard, blank-never-clears, sheet-wins, garbage log, the resync O write and the stale-row ID check. |
