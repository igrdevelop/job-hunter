...
hunter/
  ...
  profile_render.py          Deterministic Profile -> candidate.yaml /
                            ...
                            including DELETING a file the current profile no
                            longer needs (a dropped track's stale
                            `base_cv_<track>.md`, or `generation_rules.local.md`
                            once `generation_notes` is cleared back to empty),
                            not just leaving it unlisted in the returned
                            `written` paths (2026-08-30 fix — a stale
                            `generation_rules.local.md` would otherwise keep
                            being spliced into every future generation prompt,
                            since `gen_prompt.py::_local_tail()` only checks
                            the file's existence).
  profile_parse.py           Resume text extraction + LLM parsing into a
                            Profile (docs/RESUME_PROFILE_STORE_PLAN.md M3).
                            `extract_resume_text(path)` checks the file
                            exists up front (a missing `.pdf` used to report
                            the misleading "No extractable text" instead of
                            "File not found", since `hunter.pdf_text`'s
                            extractor is best-effort and swallows every
                            exception into `""`), then reads `.docx`
  ...
  profile_preview.py         Deterministic, $0, NO-LLM "test resume" preview
                            (docs/PROFILE_PAGE_TABS_WORKORDER.md, the bot-repo
                            work item — site Tab 4). `build_preview_content
                            (profile, track)` assembles a generic no-vacancy
                            content.json-shaped dict straight from a Profile:
                            track resolution reuses hunter.profile_render's
                            own private helpers (`_variant_skills`,
                            `_visible_roles_for_track`,
                            `_role_bullets_for_track` — bullets_by_track wins
                            wholesale, Role.tracks hides a role from other
                            tracks) for a real track key, but `track="core"`
                            is a deliberate special case that returns every
                            role/bullet/skill UNFILTERED — same "core is
                            facts, not one track's presentation" philosophy
                            as render_profile_md()/employers.history, which
                            also ignore Role.tracks/Bullet.tracks. Every field
                            only an LLM would supply for a real vacancy
                            (cover letters, about_me, ats score) is simply
                            omitted — generate_docs.py already treats them as
                            optional. `resume_en.skills` is matched against
                            the fixed 4-category document layout
                            (frontend/tools/methodologies/languages) by
                            case-insensitive label — a profile category whose
                            label doesn't match one of those four is not
                            shown in that section, the same limitation the
                            real LLM-driven pipeline has for any category a
                            user's own generation.yaml doesn't also
                            configure. `render_preview(profile, track,
                            out_dir, extra_env=...)` writes content.json then
                            drives generate_docs.py as a subprocess with
                            `--no-tracker` (never a tracker row — a preview is
                            not an application) and env `GENERATE_ABOUT_ME_PL
                            =false` (generate_docs.py's about-me step is an
                            LLM call and defaults ON even outside `--full`
                            mode; the preview must stay LLM-free), returning
                            the written file paths (PDF first, content.json
                            itself excluded). `validate_track()` rejects
                            anything that isn't a simple lowercase slug
                            ('core' or a track key) before it is ever used as
                            a path component — the payload crosses a process
                            boundary via a shared DB row. Consumed by
                            hunter/schedules/profile_jobs.py's `preview` kind
                            (bot side) and tools/preview_profile.py (CLI seam,
                            single-user/dev).
  profile_jobs.py            Atomic claim/finish/fail primitives over the shared
                            `profile_jobs` queue table (docs/
                            RESUME_PROFILE_STORE_PLAN.md step 4b; DDL mirrored
                            idempotently in hunter/db.py, contract source
                            job-hunter-api/docs/RESUME_PROFILE_STORE.md — the
                            API writes rows, this module is the sole
                            consumer, same precedent as hunter/users.py's
                            `telegram_link_codes`). `claim_next_profile_job()`
                            is one `UPDATE…RETURNING` (pending → running,
                            `updated_at` stamped), mirroring
                            `tracker.claim_pending`'s own primitive;
                            `finish_profile_job`/`fail_profile_job` write the
                            terminal `done`/`error` status + `result`/`error`;
  ...
  apply_failures_log.py     M4 fail-audit log (docs/HUNT_APPLY_SPLIT_PLAN.md): 
                            ...
                            NOTE: the `error` field is the HEAD of the output on the worker
                            path (`log_apply_failure` keeps the first 500 chars of the full
                            stderr/stdout) but the TAIL on the manual path (the caller passes
                            `detail[-600:]`). docs/APPLY_FAILURE_QUEUES_PLAN.md M2 fixes it
  failure_signature.py      `signature(error_text)` — a stable one-line signature for an
                            apply-failure error (docs/APPLY_FAILURE_QUEUES_PLAN.md M0): URLs,
                            UUIDs/hex ids, numbers, quoted fragments (may span newlines) and
                            paths become placeholders, then the MOST FREQUENT error-keyword
                            line wins, ties going to the last. Frequency first, because the
                            logged text is truncated at a fixed length and its edge line is
                            often a partial copy of a line repeated above; picking the last
                            line blindly split one defect into a signature per vacancy. The
                            keyword regex covers CamelCase exception names
                            (`ModuleNotFoundError` — `\berror\b` alone never matched them).
                            A text with no error line → `UNINFORMATIVE` (the stdout-head
                            case). Pure, stdlib, never raises. Shared by
                            `tools/fail_signatures.py` (M0) and the planned M2 classifier,
                            so the calibration is exactly what the classifier runs on
  cli_canary.py             Post-start Claude CLI canary (docs/APPLY_FAILURE_QUEUES_PLAN.md
                            M1). Once per bot start (`_post_init` → `cli_canary.start(app)`, a
                            plain asyncio task, not `app.create_task`), when
                            `CLI_CANARY_ENABLED` and `llm_client.cli_credentials_present()`:
                            one trivial `claude -p` built by `apply_cli._cli_argv` — the SAME
                            builder `_build_cli_command` uses, so the probe carries the real
                            apply's tool flags in the real ORDER, cwd and env. Pass → log only.
                            Fail → one Telegram alert; a transient failure (non-zero exit,
                            timeout, unexpected reply) is retried once after 60 s, a
                            deterministic one (the CLI rejecting its own argv —
                            `matches no known tool`/`unknown option` — or a missing binary) is
                            not. Wrapped in `best_effort("apply.cli_canary")`. Exists because
                            prod runs on the paid API and `main_cli` is only reached as the
                            outage fallback, where a failure exits 46 (`llm_outage`) and never
                            reaches `apply_failures.jsonl`: the 2026-09-10..21 argv break left
                            ZERO lines in the failure log (M0). Blocks nothing
  metrics.py                Per-run metrics log for the apply pipeline (docs/
                            improvement-2026-09/08-DATA_EVAL_PLAN.md M1, shipped in #267;
                            entry added 2026-09-22 with docs/PIPELINE_VIZ_PLAN.md). Two
                            lazily-created tables in tracker.db (same self-contained
                            pattern as `source_health`/`postings_seen`, NOT part of
                            `init_db()`): `generation_runs` — one row per apply attempt
                            (pipeline/profile/model, track, posting language, source,
                            manual/force flags, ATS pre-score, PDF score, verdict
                            first/final, refine rounds/accepted, judge + lang-gate
                            ...
                            **Orphan runs (closed
                            ...
                            parent's timeout (`apply_service` — `asyncio.wait_for` +
                            `proc.kill()`), an unhandled exception, or a `sys.exit` on a
                            path without `finish_run` used to leave the row with
                            `finished_at IS NULL` forever — the M0 snapshot found 5 such
                            rows on prod, days old, all `pipeline='api'`, that the
                            planned page would show as "in progress" indefinitely. Two
                            layers, both best-effort, neither changes what the apply
                            does or the outcome the caller sees: (1) parent-side stamp —
                            `finish_open_runs_for_url(url_norm, outcome, exit_code=None,
                            *, older_than_sec=0) -> int` stamps `finished_at` +
                            `outcome` (+ `exit_code`) on every open, non-backfill run of
                            that url_norm; called from `hunter/services/apply_service.py::
                            _settle_orphan_run` inside the `_after_exit` closure of BOTH
                            `run_apply_agent_subprocess` and `run_apply_agent_for_url` —
                            the ONE call site the queue worker (`apply_worker.
                            _resolve_outcome`), the inline hunt batch/retry loop
                            (`main._run_apply_agent`) and the manual paste/Apply-button
                            runner (`bot.apply_runner`) all share — with the outcome the
                            parent already resolved, prefixed `orphan:` (`orphan:fail`,
                            `orphan:cli_timeout`, `orphan:llm_outage`, …; `ORPHAN_PREFIX`)
                            so a reader can always tell a parent-written outcome from
                            one the pipeline wrote itself. A run that finished normally
                            has `finished_at` set and is untouched (the common case,
                            returns 0); an empty url_norm (paste mode, no URL) is a no-op
                            by design — an empty key must never stamp every url-less row.
                            (2) sweeper — `reset_stale_open_runs(timeout_sec) -> int`
                            stamps `orphan:stale` (`STALE_OUTCOME`) on any open run whose
                            `started_at` is older than `timeout_sec`; called every 15 min
                            from `schedules/apply_queue.py::scheduled_reset_stale_claims`
                            with `APPLY_AGENT_CLI_TIMEOUT_SEC` (the widest legitimate
                            run, so a genuine 2 h CLI run is never stamped), regardless
                            of `APPLY_QUEUE_ENABLED` — this catches a parent that died
                            with its subprocess and the paste-mode runs layer (1) cannot
  ...
  hunt_runs.py              Hunt funnel memory (docs/PIPELINE_VIZ_PLAN.md M1, 2026-09-22):
                            ...
                            actually written), `applied_inline` (handed to
                            `_auto_apply_all`), `duration_ms`. Exists because M0 on prod
                            showed the funnel CANNOT be derived afterwards: `source_runs`
                            counts a listing once per sweep, `postings_seen` once ever — a 94%
                            gap, so "found 412 → filtered 371 → dup 29 → new 12" has to be
                            written when the loop holds it. Counts only — never a job, URL or
                            title; no `user_id` (the hunt is one process for every user, and
                            `hunter/erasure.py` correctly skips the table). Own lazy-ensure DDL
                            (same self-contained pattern as source_health / postings_seen, NOT
                            part of `init_db()`), pruned to `HUNT_RUNS_KEEP` inside every
                            write like `source_health._prune`. `record_hunt(**fields)` RAISES on
                            a broken DB — the caller wraps it in `best_effort("hunt.record")`,
                            same contract as `postings_seen.record_listings`; `recent_hunts
                            (limit)` / `sum_window(since_iso)` (totals per count column +
                            merged reasons, `hunts` count) are the read side, no reader
                            wired yet (M2's `hunter/pipeline_snapshot.py`). Write site: the
                            idempotent `_flush_hunt_run` closure in `main._run_hunt_impl` —
                            called explicitly right before an inline apply batch (a batch can
                            hold `_hunt_lock` for hours and must not delay the row) and from a
                            `finally` around `_report_and_act` (the Telegram report + Step 4,
                            moved verbatim into its own function so ONE try/finally covers the
                            ACT step's seven exits — early returns and exceptions alike, since
                            the funnel numbers are real either way). No row when the hunt
                            bails before dedup (tracker read failure) or ran zero sources.
                            `tests/conftest.py` repoints `hunt_runs.DB_PATH` for EVERY test
                            (autouse) — a dozen hunt-loop tests drive `run_hunt` without the
                            `tracker_db` fixture and would otherwise append rows to the repo's
                            own ./tracker.db (the same thing `source_health` / `postings_seen`
                            still do from those tests today)
  ...
  gsheets_client.py         Low-level Sheets API v4 wrapper. Writes stay inside A–K
                            (`COLUMNS`); `read_all` reads through column O and adds an
                            `Outcome` key, the only non-A–K value the pull consumes
  outcome_writer.py         Sheet column O "Outcome" — the fifth non-overlapping writer
                            (A–K push / L sent_normalizer / M cost_writer / N verdict_writer /
                            O). Unlike M and N it round-trips: `write_outcome_cell_sync`
                            (raises on a Sheets error so `resync_dirty` keeps the row dirty;
                            skips a blank label unless `allow_blank`, so a resync of a row
                            with no recorded outcome never wipes a label the owner just typed),
                            `ensure_outcome_column_sync` (O1 header + a strict ONE_OF_LIST
                            dropdown of `tracker.OUTCOME_LABELS`, once per process; a failed
                            dropdown never blocks the write), `parse_outcome_cell` (pull side:
                            label / "" for blank / None for garbage). Sheets API only — the
                            callers own every DB read
  ...
  commands/                 One file per Telegram command handler
    ...
    outcome.py              /outcome — record what happened to a sent application (docs/
                            ...
                            (pinned by a test). Both the command and the button push the
                            label into Sheet column O at once (`gsheets_sync.mirror_outcome`,
                            best-effort — the 5-min `resync_dirty` retries a miss)
  ...
  schedules/                One file per JobQueue callback
    ...
    apply_queue.py          scheduled_reset_stale_claims (every 15 min) — two sweeps on
                            one tick. (a) M1 crashed-worker recovery, guarded by
                            `APPLY_QUEUE_ENABLED` inside the callback:
                            ...
                            forever (docs/HUNT_APPLY_SPLIT_PLAN.md). (b) Orphan
                            generation_runs sweep (docs/PIPELINE_VIZ_PLAN.md M1,
                            2026-09-22), NOT guarded by the queue flag:
                            `metrics.reset_stale_open_runs(APPLY_AGENT_CLI_TIMEOUT_SEC)`
                            stamps `orphan:stale` on any metrics run left open longer
                            than the widest legitimate wall clock — the net under the
                            parent-side stamp in `apply_service` for a parent that died
                            with its subprocess. The callback is therefore registered
                            UNCONDITIONALLY in `schedules/__init__.py` since 2026-09-22
                            (it used to be skipped entirely when the queue was off; the
                            inline hunt path leaks open runs the same way), same shape
                            as postings_prune's own in-callback guard. Each sweep has its
                            own try/except so one failing never starves the other
    profile_jobs.py          scheduled_profile_jobs_drain (every 20s) — docs/
                            RESUME_PROFILE_STORE_PLAN.md step 4b: drains the
                            `profile_jobs` queue (hunter/profile_jobs.py owns
                            the claim/finish/fail primitives; DDL mirrored in
                            hunter/db.py, contract source
                            job-hunter-api/docs/RESUME_PROFILE_STORE.md — the
                            API writes rows, this is the bot's sole consumer,
                            same precedent as `telegram_link_codes`).
                            `drain_once()` resets any `running` row stuck
                            over 10 min (a drain tick that crashed mid-job)
                            then processes every `pending` row to completion:
                            `kind='render'` → `profile_render.render_all()`
                            into `users/{uid}/candidate/`, PLUS writes
                            `profile.json` itself next to the rendered files
                            (wave-2 groundwork for a future consumer that
                            reads the structure directly);
                            `kind='parse'` → `profile_parse.extract_resume_text`
                            + `parse_resume_text` (LLM-assisted; its own
                            fallback covers a missing/failing call) on a path
                            RELATIVE to `users/{uid}/`, validated by
                            `_resolve_user_relative_path()` to stay inside the
                            user's own directory before any file is touched —
                            the payload comes from another process (the API)
                            over a shared DB row and is never trusted at face
                            value; `kind='preview'` (docs/
                            PROFILE_PAGE_TABS_WORKORDER.md, the bot-repo work
                            item) → `hunter.profile_preview.render_preview()`
                            builds a generic no-vacancy CV deterministically
                            ($0, no LLM) from the payload's own profile
                            document + `track`, into a fresh dated subfolder
                            `users/{uid}/candidate/preview/<track>/<UTC
                            timestamp>/` (history kept, never overwritten);
                            requires the user's candidate.yaml to already
...
docs/ROADMAP.md             THE index of every open plan (added 2026-09-09): actionable-now /
                            blocked-on-one-decision / SaaS product line / waiting-for-data /
...
docs/PIPELINE_SNAPSHOT_CONTRACT.md  PIPELINE_VIZ_PLAN M2, bot-repo half (2026-09-22): the
                            snapshot JSON shape job-hunter-api ports to TypeScript
                            (`GET /pipeline/snapshot`) and job-hunter-site's `/pipeline`
                            page consumes. DERIVED from `tools/pipeline_snapshot.py`, never
                            invented: the key set is the tool's `--json` output over
                            `tests/test_pipeline_snapshot_tool.py::fixture_db` — every key
                            in the output is in the doc and vice versa; when they disagree,
                            fix both in the same PR (same discipline as tracker.py's column
                            constants). Per key: type, the table + WHERE clause, window
                            mode (three: text `>= start_utc` for the `+00:00`-stamped
                            `ts`/`started_at`/`first_seen`/`last_seen` columns;
                            `date IN (Warsaw calendar days)` for `applications.date`;
                            parsed datetimes for `outcome_at`, `sent`, `queued_at`/
                            `claimed_at` in the queue's `%Y-%m-%dT%H:%M:%SZ` shape), and
                            user scoping (`applications` always `user_id = ?`;
                            `source_runs`/`postings_seen`/`hunt_runs` global by design;
                            `generation_runs` scoped `(user_id = ? OR user_id = '')` in
                            `apply.runs` only — the card, events and coverage read it
                            unscoped). Pins the UNMEASURED/`null` conventions
                            (`hunt_runs_unmeasured`, rule `why`), `_bucket_status`, the
                            `queue_mode_observed` three-signal rule (#293),
                            `_infer_stage` incl. the refine-round branch, and the five
                            coverage rules + 3b with thresholds and verdict strings
                            verbatim. Specifies (does NOT create) the shared fixture pair
                            `tests/fixtures/pipeline_snapshot/{fixture.sql, expected.json}`
                            — the SQL is the test fixture's inserts with the clock frozen
                            at `2026-09-22T12:00:00+00:00`, the JSON was generated from
                            it and reproduces every assertion in the test file; volatile
                            fields (`generated_at`, `start_utc`, every `*_min_ago`/
                            `wait_min`/`in_min`, `at` strings, `next_slot`, local-config
                            keys) are normalised before compare. The bot-side test that
                            pins the pair needs a `now=` seam in `build_snapshot` and is
                            the next PR. "Not in the contract": `next_slot` (scheduler
                            roster), `events[].payload` (free-form; only `score`/`chars`/
                            `error`/the refine `{round, kind, score, best, reason}` are
                            stable, and only from the full column, not the 80-char footer
                            string), `coverage` ("about this data" tab at most), and
                            anything read from `.env` (`queue_enabled_local_config`,
                            `failures.next_retry`, `timeout_sec`). One caveat left open:
                            `applications.date` is `date.today()` of the writing process,
                            and whether the prod container's local day is Warsaw or UTC
                            was not verified — matters only for the 1-day window.
...
docs/improvement-2026-09/   Improvement plan series (2026-09-09): eight-perspective
                            analysis on the way to SaaS — product/marketing/architecture/
                            engineering plans plus security/ops/compliance/data audits, ten
                            further experts (09), and a single execution order + dependency
                            map in its README. Sits BEFORE docs/SAAS_PIVOT_PLAN_supersedes_
                            PYTHON_CORE_PLAN.md Stage 1 (product phase 0) and amends its order;
                            the pivot plan stays the executable document. ROADMAP.md §0 indexes it
...
tools/parse_resume.py       CLI seam for hunter/profile_parse.py (docs/
                            ...
                            when the file itself can't be read. Both this
                            file and `render_profile.py` below reconfigure
                            stdout/stderr to UTF-8 on platforms that support
                            it (matching `tools/preview_judge.py`) and pass
                            `ensure_ascii=False` on their JSON output,
                            matching the rest of the codebase's JSON-write
                            call sites — closes a Windows `UnicodeEncodeError`
                            crash risk on a non-ASCII path/field
tools/render_profile.py     CLI seam for hunter/profile_render.py (docs/
                            ...
                            Exit 1 + stderr when the input file is missing,
                            not valid JSON, or `render_all()` itself raises
                            `OSError` (out_dir collides with an existing
                            file, a permissions error) — that write step is
                            wrapped in its own try/except so a failure there
                            gets the same clean "ERROR: ..." message as every
                            other failure path, not a raw traceback. Reads
                            the profile with `utf-8-sig` (tolerates a
                            Windows-authored BOM). Calls
                            `profile_schema.validate()` after parsing and
                            prints any problems as a stderr `WARNING:` — a
                            malformed/empty profile still renders (this CLI
                            transforms data, it doesn't gate content quality,
                            that's the site's confirmation screen's job), but
                            a caller no longer mistakes a blank
                            `candidate.yaml` for a clean success
tools/preview_profile.py    CLI seam for hunter/profile_preview.py (docs/
                            PROFILE_PAGE_TABS_WORKORDER.md, the bot-repo work
                            item): `python tools/preview_profile.py
                            <profile.json> <out_dir> [--track angular]`
                            builds a generic no-vacancy content.json for the
                            requested track ('core' default) and renders it
                            via generate_docs.py, printing
                            `{"written": [...]}`. Deterministic, $0, no LLM
                            call. Runs against the CURRENT process's
                            candidate.yaml resolution (single-user/dev, same
                            as tools/render_profile.py) — the per-user,
                            multi-user path is hunter/schedules/
                            profile_jobs.py's `preview` job kind. Exit 1 +
...
tools/fail_signatures.py    Read-only M0 measurement (docs/APPLY_FAILURE_QUEUES_PLAN.md):
                            groups `logs/apply_failures.jsonl` (+ rotated `.1`–`.5`) by
                            `hunter.failure_signature.signature` and reports per signature:
                            records, DISTINCT vacancies, first/last seen, peak distinct
                            vacancies in any 6 h window, cli_mode share, exit codes,
                            source-domain spread. `--db tracker.db` (opened `mode=ro`) adds
                            each affected URL's fate now: fail_retryable / given_up
                            (`fail_count >= MAX_FAIL_RETRIES`) / queued / applied /
                            skipped/expired/manual — the damage count for an incident.
                            Prints the plan's four decision rules; the known 2026-09-10
...
tools/pipeline_snapshot.py  Read-only M0 measurement (docs/PIPELINE_VIZ_PLAN.md): ONE
                            ...
                            **Reads the M1 data since 2026-09-22** (PRs #288–#291; still
                            ...
                            (same arithmetic as `hunt_runs.sum_window`, only
                            `COUNT_COLUMNS` is imported — never the module's writer path)
                            and is the PRIMARY funnel line in the text report ("found →
                            filtered → dup → new → queued"), `source_runs`/`postings_seen`
                            demoted to secondary; `apply.pending.oldest_wait_min` + a
                            `wait_min` per head entry from `queued_at` (HEAD-row rule as
                            `tracker.oldest_pending_wait_min`: a NULL at the head reports
                            None, never skips ahead); the in-progress card reads the
                            `start` event (`stage_started_min_ago`) and the latest
                            refine-round event (`refine_progress` = round/kind/score/best/
                            outcome) instead of inferring — `_infer_stage` treats a
                            refine `accepted`/`rejected`/`discarded` as "still in refine"
                            and keeps the pre-M1 next-stage guess for runs without those
                            events; coverage adds `3b_hunt_runs_present` (PASS with the
                            hunts-in-window count; UNMEASURED when the table is missing —
                            the derived-gap rule 3 stays for the pre-M1 comparison) and
                            `orphan_stamped` on rule 5 (informational: runs whose outcome
                            the parent/sweeper stamped `orphan:*`). A pre-M1 DB copy still
                            snapshots — `hunt_runs: None`, `oldest_wait_min: None`, the
                            M1-only card fields None.
                            `tests/test_pipeline_snapshot_tool.py` builds a real-schema
                            fixture DB (`init_db` + the four lazy DDLs) and pins every
                            count, every rule verdict and the read-only contract; the
                            fixture deliberately mixes pre-M1 finished runs with an
                            M1-shaped in-progress run so both branches stay covered
...
scripts/                    Host-side ops scripts (tracked, sh — LF via .gitattributes).
                            Never executed by an agent or by the app itself; the owner
                            installs the two backup scripts via host cron (docs/
                            DEPLOY.md "Backups"), while `docker_prune.sh` is ALSO run
                            by the deploy workflow on every master push (see its entry).
  ...
  docker_prune.sh            Bounded retention for the job-hunter deploy images
                            (2026-09-12): keeps the image behind the live
                            `job-hunter` container + `KEEP_PREVIOUS` (default 1)
                            newest other `ghcr.io/igrdevelop/job-hunter` images as
                            a rollback target, `docker rmi -f`s the rest (images
                            used by ANY running container are skipped), then runs
                            the generic `docker image prune -a -f --filter
                            until=168h` for the sibling projects' leftovers on the
                            shared daemon. Exists because a TIME window cannot cap
                            the disk: the image is ~8.1 GB uncompressed (Playwright
                            chromium), and on 2026-09-12 ten of them younger than
                            two days filled the 75 GB VPS to 100% while both the
                            deploy-time `until=168h` prune and the identical host
                            cron ran and freed nothing. Unlike the two scripts
                            above this one IS executed by CI: `.github/workflows/
                            deploy.yml` curls it at the deployed commit SHA
                            next to docker-compose.yml and runs it before `docker compose
                            pull` (reclaim) and after `up -d` (settle to live + 1);
                            the host cron in docs/DEPLOY.md "Disk hygiene" runs
                            the same local copy nightly. POSIX sh, `DOCKER=` env
                            for cron's minimal PATH; every docker listing is
                            captured on its own line (no `pipefail` in sh, so a
                            failed `docker images` must not degrade into a silent
                            "nothing to remove"); a failed `rmi` only warns — the
                            deploy's free-space gate (12000 MB, the real image +
                            headroom) is the actual protection.
  ...
  commands/
    ...
    apply.md                Generate a tailored application package. NOT just a desktop
                            ...
                            **Explicit tool policy, not full access** (2026-09-10,
                            ...
                            a thin wrapper over `_cli_argv(prompt)` — shared with the
                            post-start canary, `hunter/cli_canary.py`; the prompt MUST precede
                            the two variadic tool flags, see the 2026-09-21 work-log entry)
...
.coderabbit.yaml            CodeRabbit PR-review config (tracked; same file exists in
                            job-hunter-api / job-hunter-site). First line of automated
                            review on PRs — carries a DIGEST of this file's invariants
                            (candidate.get neutral defaults, best_effort wrapping,
                            lock regeneration, five source-registration points) so PRs
                            opened outside Claude Code get flagged too. CLAUDE.md stays
...
| `CLI_CANARY_ENABLED` | `true` | Post-start Claude CLI canary (`hunter/cli_canary.py`, docs/APPLY_FAILURE_QUEUES_PLAN.md M1): once per bot start, when a CLI login exists, one trivial `claude -p` through the apply pipeline's own argv builder; one Telegram alert on failure. One flat-cost subscription call per restart, no API spend. `false` disables it. |
...
| `TZ` *(docker-compose.yml, not `.env`)* | `Europe/Warsaw` | Container local time. Set in the compose `environment:` block on 2026-09-24 so every `date.today()` / naive `datetime.now()` in the bot — the tracker `date` column, `Applications/<date>/` folders, the daily summary's "yesterday", funnel cutoffs, backup filenames — agrees with `SCHEDULE_TIMES`/`SCHEDULE_BLACKOUT`, which were already Warsaw. Before it the container ran in UTC and a row written 00:00–02:00 Warsaw carried the previous day. Every stored UTC timestamp (`claimed_at`, `queued_at`, metrics, `source_runs`, `postings_seen`, `hunt_runs`, `apply_failures.jsonl`) is written with an explicit tz and is unaffected; `hunter_errors.log` asctime shifts to Warsaw. No backfill of older `date` values. |
...
| `CLAUDE_CODE_OAUTH_TOKEN` *(CLI outage fallback)* | — | M4/M4b: on an LLM account outage, calls are served through the Claude CLI (Pro subscription) instead of failing. **Live in prod since 2026-07-29** (OAuth login done, `claude -p` smoke-tested in the container). **No feature flag** (owner decision 2026-07-18): the switch is whether the CLI can AUTHENTICATE (`llm_client.cli_credentials_present`), which is true for either a long-lived `CLAUDE_CODE_OAUTH_TOKEN` in the environment (checked first — `claude setup-token`, valid ~1 year, does not rotate) or an OAuth login in the mounted `./.claude-cli/` volume. Turning the fallback off takes BOTH: unset the variable and empty the volume (`claude /logout`). Two layers: (1) inside `llm_client.call_llm` — ANY call (generation, judge, verdict, refine, translate, outreach) hit by `LLMOutageError` gets ONE `claude -p` retry with the same prompt via stdin, **pinned `--model <requested>`** (2026-08-10 fix: unpinned calls were served by the subscription's DEFAULT model — the verdict judge stopped being Haiku and CLI-served verdicts averaged ~3 points below API-served ones; if the subscription rejects the pinned model, ONE unpinned retry runs — a default-model answer still beats re-raising; no unpinned retry on missing binary/timeout) (dual-apply shadow excluded — a substituted model would poison the A/B; CLI failure re-raises the ORIGINAL outage so exit-46 semantics hold); (2) `apply_agent.main()` pipeline-level retry via `main_cli` as the second line. Dispatch is fixed: `LLM_API_KEY` set → **paid API primary** (the old "CLI detected → try CLI first" auto-preference is REMOVED — desktop bare runs now cost API money; use `--cli`/`APPLY_USE_CLI=true` for subscription-only, `tools/preview_apply.py` passes `--cli` explicitly), switches back automatically once topped up; no key → CLI-only. Activation was a one-time `docker compose exec -it job-hunter claude` OAuth login on the deploy host (done 2026-07-29). **Prod moved to `CLAUDE_CODE_OAUTH_TOKEN` on 2026-09-08** (docs/DEPLOY.md "Claude CLI token"): a failed refresh does not delete `.credentials.json`, it rewrites it with BLANK tokens, so that day the login was silently dead while the API balance was also drained — both LLM paths down, auto-apply paused ~18 h, and a presence-only credentials check still reported "logged in" and burned a doomed `claude -p` per outage-hit call. Hence `_credentials_file_usable()` (a file that positively proves empty tokens no longer counts; anything unparseable still fails OPEN) and the token path, which cannot be blanked by a background refresh. Either way these are personal subscription credentials — treat like `gsheets_token.json`, never commit. CLI-served calls record no usage → Cost $ understates outage-window runs. |
...
3b. **hunt_runs row** (Step 3b in the code — after dedup, around the ACT step;
   `hunter/hunt_runs.record_hunt`, docs/PIPELINE_VIZ_PLAN.md M1): ONE row per
   hunt with the funnel the Telegram report shows — found, filtered_out +
   reasons, dup_url / dup_ct / dup_cooldown, new, plus what the ACT step then
   decided (capped by `MAX_JOBS_PER_RUN`, queued as PENDING, or handed to the
   inline batch) and the hunt's duration. Written by an idempotent flush:
   explicitly right BEFORE an inline apply batch (which can hold the lock for
   hours), otherwise from a `finally` around the report + ACT step, so every
   exit — early return or exception — still records the numbers. Inside
   `best_effort("hunt.record")`; `HUNT_RUNS_ENABLED` gates it; counts only,
   never a job/URL/title; nothing reads it back into the hunt. No row when
   the hunt bails on the tracker read before dedup.
...
post-processing runs, the documents are rendered AND the applied row exists.

That same fact makes **`--no-tracker` mandatory on every post-generation
re-render in `main_cli`** — the language gate (step 5b's CLI branch), the
foreign-contact drop, the NBSP self-heal and the verdict-refine loop all
re-render documents over a row that already exists, so none of them may write
it: they pass `force=skip_dedup`, and in `/force` mode a tracker write does
DELETE+INSERT (new sync ID, false Re-application flag). The rule was written
down only for the refine loop when that loop introduced it; the language gate
and the self-heal were still rewriting the row until 2026-09-12. `--force`
itself is inert once `--no-tracker` is passed (`generate_docs.py` uses
`force_mode` nowhere but the tracker write it skips), so leave the flag alone
rather than "unifying" it away. Pinned by
`tests/test_apply_cli_regen_no_tracker.py`, whose sweep requires EVERY
`build_generate_docs_cmd` call in `apply_cli` to carry `no_tracker=True` — the
initial render belongs to the CLI skill, so any call site here is by definition
a re-render.

Its four abort stages -- React-only stack, company+title dedup, judge block,
...
| — | Outcome (`outcome_label` + `outcome_at` DB columns) | What actually happened to a SENT application: one of `tracker.OUTCOME_LABELS` = `interview` / `rejected` / `offer` / `silence`, empty = not recorded (docs/improvement-2026-09/08-DATA_EVAL_PLAN.md M1, owner decision 2026-09-12). Added after a 90-day prod funnel run showed 399 sent applications and ZERO recorded outcomes — nothing ever wrote the free-text `answer` column, so no metric past "sent" was measurable. Written by `tracker.set_outcome(url_or_id, label)` (user-scoped, stamps `outcome_at`, sets `sheets_dirty=1`) via `/outcome`. `silence` is an OBSERVED outcome but not a reply: `funnel._is_answered` counts only `OUTCOME_REPLY_LABELS` (plus the legacy `answer` text), while `funnel._has_outcome` counts all four, so `answered == 0` can be told apart from "nobody recorded anything". Round-trips through Sheet column **O** (`hunter.outcome_writer`, dropdown of the four labels): pushed straight after `/outcome` (`gsheets_sync.mirror_outcome`, which also writes a blank for `clear`) and by `resync_dirty` for every dirty row carrying a label — a failed O write keeps the row dirty; pulled by `gsheets_sync._merge_outcomes` (see the Outcome conflict matrix below). O stays outside `gsheets_client.COLUMNS` on purpose, so the A–K push can never blank a label the owner typed. |
| — | ATS Verdict (`ats_verdict` DB column) | Independent PDF-verdict score (0–100): one `JUDGE_MODEL` (Haiku) call over the text extracted from the rendered EN CV PDF. Stamped post-hoc by `tracker.set_ats_verdict` (apply Step 7.7; the row already exists). NULL = no verdict. Mirrored to Sheet column **N** by `hunter.verdict_writer` when the bot-process `mirror_new_row` runs (the verdict is in the DB by then); `tools/sync_verdicts.py` backfills misses. Five non-overlapping Sheet writers: A–K main push, L sent_normalizer, M cost_writer, N verdict_writer, O outcome_writer. |
| — | Skip reason (`skip_reason` DB column) | WHY a SKIP row was written (docs/MARKET_MEMORY_PLAN.md M2, 2026-09-13): `<prefix>[:<detail>]` over `tracker.SKIP_REASON_PREFIXES` — `button` (Skip button), `doomed:<rule>` (doomed-gate HARD finding), `prescreen`, `react`, `dedup_ct` (the API pipeline's Step 4.55 company+title gate), `abort:<reason>` (every CLI-pipeline post-generation abort incl. its company+title dedup, which lands as `abort:company+title dedup (...)` — the uniform abort rule), `other[:<name>]` (e.g. `other:backend_only`). Canonicalised by `normalize_skip_reason`; an unknown prefix becomes `other:<original>` — a write never fails over a label. '' = pre-M2 row or an untagged writer. Stamped by `add_skipped`/`add_react_skipped` and both in-place conversions; full call-site → value table in `tests/test_skip_reason.py`. **Not mirrored to the Sheet**: the Sheets layer is the fixed A–K contract plus four single-column writers (L sent, M cost, N verdict, O outcome), and the owner reads reasons through reports/`tools/`, not the Sheet — a fifth writer for a reports-only column is more round-trip surface than it is worth. No gate reads it. |
...
3. User edits Sent date / To Learn / Re-application / Outcome (column O) in Sheets
...
2. `_apply_pull_delta_db()` — conflict matrix for `Sent`/`To Learn`/`Re-application`
   ...
   then `_apply_outcome_pull_db()` — column O, its own rule below (runs after the
   insert, so a self-healed row takes its Sheet outcome too). `tracker.apply_pulled_outcomes`
   stamps `outcome_at` but leaves `sheets_dirty` alone — the Sheet already shows it.
...
### Conflict matrix (Sent / To Learn / Re-application, columns A–K)
- DB row `sheets_dirty=1` → keep the DB outright for all three columns (added
  2026-09-14, `tracker.get_pull_dirty_ids()` + `_apply_pull_delta_db`): the row
  holds a Sent/To Learn/Re-application edit — most commonly job-hunter-api's web
  UI (My Status), which sets these columns and `sheets_dirty=1` directly in
  `tracker.db` — that `resync_dirty()` hasn't pushed to the Sheet yet (~5-min
  cycle). Without this, a pull landing first (every ~30 min, at bot startup, or
  on `/sync_sent`) read the still-blank Sheet cell as authoritative, silently
  reverted the fresh web edit, and the next resync then pushed that same blank
  back out — permanently erasing the edit with no error anywhere. If the Sheet
  ALSO changed in the meantime (owner hand-edited the cell before the dirty
  ...
  next pull after resync clears the flag; a per-column dirty flag would close
  it and was not judged worth a migration at this volume (same call as the O
  column's version of this gap).
- Else, bot wrote a terminal marker (EXPIRED, or the `—` dash every SKIP/FAIL
  row gets at insert since 2026-08-08), Sheets is empty → keep the marker
  (Sheets will be fixed by resync — a blank cell there means the marker
  wasn't pushed yet, not that the user cleared it)
- Else, Sheets has date / was edited → trust Sheets
- Else, To Learn, Re-application → always trust Sheets (user edits there)

### Conflict matrix (Outcome, column O)
- Valid label (any case/whitespace) that differs from the DB → trust Sheets, same side
  as To Learn
- Blank cell → keep the DB. Clearing is `/outcome <id> clear` only: rows labelled
  before the O mirror existed, or whose O write was lost, show a blank cell that is
  not a deletion
- Not one of `tracker.OUTCOME_LABELS` → warning log, ignored; never raises, never clears
- DB row `sheets_dirty=1` → keep the DB: it holds an `/outcome` change the Sheet hasn't
  received yet (resync pushes it within 5 min). Known gap, same shared-flag caveat as
  the A–K matrix above: an owner edit in O on a row that turns dirty for another reason
  before the next pull is overwritten by the DB value when that row resyncs
...
| 2026-09-22 | fable | **`hunt_runs` table — docs/PIPELINE_VIZ_PLAN.md M1, first item (branch `feat/hunt-runs-table`, cut from `origin/master`).** M0 on prod (2026-09-22) failed rule 3: `source_runs` counts a listing once per SWEEP (49,356 over 7 days), `postings_seen` once EVER (2,825 unique url_norm) — a 94% gap, so the planned page could never show "found 412 → filtered 371 → dup 29 → new 12" from those tables. The numbers already exist as locals of `hunter/main.py::_run_hunt_impl` and went to one Telegram message; now they are persisted once per hunt, $0. **`hunter/hunt_runs.py`**: self-contained lazy-ensure module in the exact `source_health`/`postings_seen` style (own `_DDL`, `_ensure_table`, `DB_PATH = TRACKER_DB_PATH`, NOT in `init_db()`); columns `ts` (UTC ISO seconds — the hunt's START, passed in by the loop), `trigger`, `sources` (JSON), `found`, `filtered_out`, `filter_reasons` (JSON, non-zero only), `dup_url`, `dup_ct`, `dup_cooldown`, `new`, `capped`, `queued`, `applied_inline`, `duration_ms` — `"trigger"` and `"new"` are SQLite keywords and are quoted everywhere. `record_hunt(**fields)` RAISES on a broken DB (the caller's `best_effort("hunt.record")` needs the exception to count), prunes to `HUNT_RUNS_KEEP` (config, default 2000 ≈ 3 weeks at ~100 slots/day) inside every write; `recent_hunts(limit)` / `sum_window(since_iso)` are the read side, no reader wired yet (M2). `HUNT_RUNS_ENABLED` (default true) in config + `.env.example`. **Wiring**: `run_hunt` grew a `trigger` kwarg, derived from `notify_queued` when not given (only the `/hunt` command sets it → `manual`, else `scheduled`; `/force` never runs the hunt loop at all, so `force` is reserved). The Telegram report + Step 4 (ACT) of `_run_hunt_impl` moved VERBATIM into `_report_and_act(...)` so ONE try/finally covers the ACT step's seven exits — the alternative was a flush call before each early return. An idempotent `_flush_hunt_run` closure is called explicitly right BEFORE the inline `_auto_apply_all` batch (it can hold `_hunt_lock` for hours and must not delay the row; `applied_inline` = handed to the batch, per-job outcomes are the tracker's) and from the `finally` otherwise; `queued` counts `add_pending` calls that actually returned. No row when the tracker read before dedup fails or zero sources ran. No hunt step, message or early return changed. **Tests** (`tests/test_hunt_runs.py`, 16): module round-trip / prune / `sum_window` on a tmp DB, plus real `run_hunt` runs over a four-job fixture (one filtered, one dup-URL, one dup-company+title, one new) asserting the row equals the numbers regex-parsed back out of the Telegram summary; queue path (`queued=2`, `capped=1` under `MAX_JOBS_PER_RUN=2`), inline path (row exists when the batch starts, and only ONE row after the finally), flag off, DB-read failure, and a `best_effort` failure count + recovery. Mutation-verified by hand, three cuts: `dup_ct=dup_url + 1` (fails `2 == 1`), dropping the `queued += 1` (fails `0 == 2`), dropping the pre-batch flush (fails `[0] == [1]`). **Test hygiene**: a new autouse fixture in `tests/conftest.py` repoints `hunt_runs.DB_PATH` to a per-test file — a dozen hunt-loop tests drive `run_hunt` without `tracker_db` and would otherwise append a row to the repo's own ./tracker.db on every run (which `source_health`/`postings_seen` still do from those tests today — pre-existing, not touched here). Two `test_hunt_queue_delivery.py` fakes of `_run_hunt_impl` gained the `trigger` kwarg. Gates: ruff clean, 3767 fast tests pass, mypy ratchet 224 → 205 (no regression, 0 errors in the two touched modules). Not pushed, no PR — the branch is reported for review. |
