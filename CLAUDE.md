...
| `LLM_OUTAGE_FALLBACK_CLI` | `false` | M4: on an API account outage, retry the vacancy ONCE through the Claude CLI (Pro subscription) before reporting exit 46. When ON, the CLI is reserved as the fallback (the "CLI detected → try CLI first" auto-preference in `apply_agent.main()` is skipped, so the paid API stays primary). Inert until the ops half is done (CLI in the Docker image + logged-in `~/.claude` volume — see the plan's M4 checklist; a personal subscription token on the deploy host is an explicit owner decision). |
