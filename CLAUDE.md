...
> `_MIN_CONFIDENCE`, `REPOST_GATE_ENABLED`, `REPOST_WINDOW_DAYS` and
> `GEN_SKIP_PL_FOR_EN` below now resolve through `hunter/gen_profile.py`
> (docs/GENERATION_ARCHITECTURE_ANALYSIS.md §6 wave 3): builtin default (the
> value shown below) < optional per-user `candidate/generation.yaml` < the env
> var itself, which still wins and behaves exactly as documented — nothing
> below changes. See the `gen_profile.py` entry in Repository Layout and
...
> `gates.react_skip_min_mentions`) exist ONLY in `generation.yaml` — they had
> no env var before this wave and still don't.
...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-27 | sonnet | **Wave 3 PR 1 of docs/GENERATION_ARCHITECTURE_ANALYSIS.md §6: added `hunter/gen_profile.py`, a per-user GENERATION profile loader, and wired 14 existing config constants through it.** Same architecture as `hunter/filter_profile.py`, applied to WHAT the pipeline writes into a CV (ATS-loop thresholds, verdict/refine rounds, judge/gate modes) instead of what the hunt filters for. `builtin_defaults()` reproduces every value verbatim — module constants from `hunter/pipeline/ats.py`, `hunter/verdict_refine.py`, `hunter/ats_pdf_roundtrip.py`, `hunter/pipeline/gates.py`, plus the 14 `config.py` env-var defaults (`ATS_VERDICT_*`/`JUDGE_*`/`DOOMED_GATE_*`/`PRESCREEN_*`/`REPOST_GATE_*`/`GEN_SKIP_PL_FOR_EN`). Priority env > YAML > builtin; unlike `filter_profile.py`, env overrides are re-applied fresh on every `load_gen_profile()` call rather than baked into the mtime cache key, so a changed env var takes effect without touching any file. `config.py`'s 14 constants resolve at import time (apply is a fresh subprocess per vacancy); the 7 constants that lived inside a function body with no prior env var now call `gen_profile.get()` at CALL time instead (`HEAL_DELTA_PP` became a `heal_delta_pp()` function). One existing test's assertion changed from grepping `config.py` source text for a literal `os.getenv(...)` string to asserting the resolved runtime value — the string legitimately no longer exists, the value is unchanged (proven by the new parity test). New: `candidate/generation.example.yaml`, `tests/test_gen_profile*.py` (52 tests: loader unit tests, parity, dotpath-coverage readiness). Full suite 2862 green, both golden E2E pass, ruff clean, mypy baseline 217 (was 223). Full writeup in docs/AGENT_LOG.md. PR 2 (document layout: font/sizes/margins/section labels/skill categories/GDPR clause) stacks on this branch. |
