...
| `LLM_OUTAGE_FALLBACK_CLI` | `false` | M4/M4b: on an LLM account outage, serve the call через the Claude CLI (Pro subscription) instead of failing. Two layers: (1) inside `llm_client.call_llm` — ANY call (generation, judge, verdict, refine, translate, outreach) hit by `LLMOutageError` gets ONE `claude -p` retry with the same prompt via stdin (dual-apply shadow excluded — a substituted model would poison the A/B; CLI failure re-raises the ORIGINAL outage so exit-46 semantics hold); (2) `apply_agent.main()` pipeline-level retry via `main_cli` as the second line. When ON, the "CLI detected → try CLI first" auto-preference is skipped — the paid API stays primary and switches back automatically once topped up. Repo side in place (CLI in Dockerfile, `CLAUDE_CONFIG_DIR`/`IS_SANDBOX` envs, `./.claude-cli:/root/.claude` volume, `.claude/commands/` re-included); to activate: one-time `docker compose exec -it job-hunter claude` OAuth login (owner decision 2026-07-18) + this flag. CLI-served calls record no usage → Cost $ understates outage-window runs. |
