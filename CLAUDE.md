...
hunter/
  ...
  pipeline/                 `hunter/apply_shared.py` is being decomposed into this
                            package, one PR at a time (docs/
                            GENERATION_ARCHITECTURE_ANALYSIS.md wave 1 — pure
                            moves, no behavior change). `apply_shared.py` keeps
                            re-exporting every symbol (incl. underscored ones)
                            for the ~32 existing import sites across the repo;
                            see its own docstring + tests/test_apply_shared_shim.py
                            for the pinned surface. So far:
...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-27 | sonnet | **Wave 1 PR B of docs/GENERATION_ARCHITECTURE_ANALYSIS.md §6: moved the content scrubs and the ATS keyword loop out of `hunter/apply_shared.py` into `hunter/pipeline/scrubs.py` + `hunter/pipeline/ats.py`.** Second of 3 PRs decomposing `apply_shared.py` (see the PR A entry below for the plan). This PR carries the one cross-dependency the migration plan flagged in advance: `ats.py`'s `_ats_check_loop` imports `_COMPLIANCE_CLAIM_RE` from `scrubs.py` (the loop strips employer-credential terms out of the job text before showing it to the rewrite prompt, using the SAME compiled regex `_strip_compliance_claims` uses to scrub the resume) and `_llm_p` from the already-moved `profiles.py` — both plain static imports, since neither is ever monkeypatched through `hunter.apply_shared`. One NEW instance of the PR-A shim-compatibility indirection: `build_ats_keyword_checklist()` and `_ats_check_loop()` both call `_filter_self_description_keywords` — moved to the SAME file (`ats.py`) as its two callers, but `tests/test_apply_shared.py::test_build_ats_keyword_checklist_caps_at_30` still monkeypatches it via `apply_shared._filter_self_description_keywords = ...` (a raw attribute assignment, not `monkeypatch.setattr`, manually restored in a `finally`) — so both callers re-import it from `hunter.apply_shared` at call time rather than relying on the bare in-module name, exactly like the notify()/APPLICATIONS_DIR cases in PR A. Verified every moved function body against `origin/master`'s `apply_shared.py` with a line-range diff script — zero differences beyond the documented one-line insertions (confirmed `_strip_compliance_claims` is byte-identical). One thing NOT touched despite looking movable: the ORIGINAL lazy `import copy` sits inside `_ats_check_loop`'s body (after the early-return checks, right before `_exp_of` is defined) — an initial draft hoisted it to a module-level import by mistake; caught before committing and put back in its exact original position, since moving an existing lazy import's timing is explicitly out of scope for this migration. Full suite 2804 green (unchanged from PR A — no new tests needed, `test_apply_shared_shim.py` from PR A already covers every name this PR re-exports), both golden E2E suites pass, `ruff check`/`format` clean. `apply_shared.py`: 1584 → 916 lines. |
| 2026-08-27 | sonnet | **Wave 1 PR A of docs/GENERATION_ARCHITECTURE_ANALYSIS.md §6: started decomposing `hunter/apply_shared.py` (1843 lines) into `hunter/pipeline/*`, pure moves behind a re-export shim.** This PR moves the 5 leaf modules with no cross-dependencies on each other: `hunter/pipeline/errors.py` (exit codes, `ApplyError`, `is_rate_limit_error`, `is_transient_fetch_error`), `profiles.py` (`_llm_p`, `_translate_p`), `notify.py` (`notify`, `send_telegram_documents`), `folders.py` (`compute_output_folder`, `_sanitize_folder_company`, `PROMPTS_DIR`, `CANDIDATE_DIR`), `validate.py` (`validate_content`, `REQUIRED_JSON_KEYS`). `apply_shared.py` re-exports all of it; everything else (scrubs, ATS loop, language gate, gates, post-generation abort) stays physically in `apply_shared.py` for now — PRs B and C move the rest. New `tests/test_apply_shared_shim.py`, written BEFORE any code moved, pins the ~45-name public surface of `hunter.apply_shared` (every name any file in the repo actually imports from it, incl. underscored ones and qualified-attribute-only access like `apply_shared.requests.post`) so the migration can't silently drop a backward-compat name. **The one non-trivial finding:** several existing tests monkeypatch specific names via the STRING PATH `"hunter.apply_shared.<name>"` (`tests/conftest.py`'s autouse `_no_telegram` fixture blanks `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` there for every test in the suite; others patch `APPLICATIONS_DIR`, `notify`, `_translate_p`, `_translate_resume`, `_filter_self_description_keywords`, `run_doomed_gate`, `send_telegram_documents`, `validate_content`). That only works because, pre-move, every function lived in one module and Python's global lookup always resolved through that module's own dict — moving a function to a new file while its callER stays where it is (or vice versa) breaks the patch silently, since the two are now different dict entries pointing at what was briefly the same object. Fix: the specific functions that read one of these names internally (`notify()`/`send_telegram_documents()` reading `TELEGRAM_BOT_TOKEN`/`_CHAT_ID`/`_SEND_DOCS`; `compute_output_folder()` reading `APPLICATIONS_DIR`) now re-import that name from `hunter.apply_shared` at call time instead of a static module-level import — a deliberate, narrow, per-name indirection, not a stylistic change; each is called out in its function's own docstring and in the CLAUDE.md `pipeline/` entry. Full suite 2804 (was 2803 + 1 net new after the shim tests) green with zero existing-test edits; both golden E2E suites (`test_golden_apply_e2e.py`, `test_golden_apply_cli_e2e.py`) pass unchanged; `ruff check`/`format` clean. Same fix pattern will recur for the callers that move in PRs B/C (`_translate_p` read from `_translate_resume`/`_translate_plain`, `notify` read from the doomed/prescreen/abort gates, `_filter_self_description_keywords` read from the ATS loop) — documented per-PR as they land, not solved in one shot here since the callers don't exist in this package yet. |
| 2026-08-27 | sonnet | **Wave 0.5 of docs/GENERATION_ARCHITECTURE_ANALYSIS.md §6: aligned the CLI pipeline with the API pipeline on four of the five quality stages §3 found missing.** Owner decision (recorded in the analysis doc 2026-08-27): production runs through the CLI subscription deliberately, and CLI/API must behave identically — so these four gaps were on the PRIMARY path, not "a defect on a quarter of runs". All four ported mechanically into `hunter/apply_cli.py`'s two existing insertion points, behavior copied 1:1 from `hunter/apply_api.py`. **(1)** Pre-LLM `is_react_only_job_text`/`is_backend_only_job_text` (API Steps 1.5c/1.5d) inserted right before `cmd = ["claude", "-p", ...]` — the only one of the four that also SAVES the generation spend, since it now aborts before `claude -p` runs instead of after the CLI skill has already rendered a full package (the existing post-generation React-only check stays, as the safety net for stacks the LLM decides on its own that the text heuristic can't catch). **(2)** `_strip_compliance_claims` added to the scrub block before `_strip_prestige_claims`, folded into the existing `_scrub_fixes` list that already drives the rewrite+regenerate path. **(3)** `content_qa.run_qa` added after the language-gate block, warn-only. **(4)** `is_bogus_company` added next to it; unlike the API pipeline (checks before the folder exists), the CLI skill has already rendered docs and written the tracker row, so the abort goes through `abort_after_generation` (the 2026-08-24 Interia-fix undo pattern) instead of a bare `sys.exit(0)`. The fifth gap, `_ats_check_loop`, was explicitly NOT ported — an owner decision, not a mechanical port: on the CLI path the documents already exist, so the loop would degenerate into a second rewrite-and-rerender on top of what verdict-refine already does. Stage (1) is placed BEFORE the manual screen / doomed gate / repost gate / prescreen (not after, as an earlier draft had it) to match `apply_api`'s own order byte-for-byte — an obvious React-only/backend-only posting is rejected by this free regex check before paying for the repost-gate TF-IDF pass or the prescreen's Haiku call, same as the API pipeline. 6 new tests (`tests/test_golden_apply_cli_e2e.py`, 13 total in the file), mutation-verified ×5 (every mutation failed the right test for the right reason). Incidentally found, confirmed pre-existing (not a regression): a test-isolation hazard in the same file's `FakeClaudeSkill` fixture where `sklearn`'s first lazy import in a process shells out to Windows `ver`, which the fixture's process-wide `subprocess.run` monkeypatch miscounts as an extra `claude -p` call — reproduces on the pre-existing `TestTheSkillIsCalledOnce` test too when run standalone-first; invisible under the normal `pytest tests/` run order. Flagged as a follow-up and fixed the same day (`FakeClaudeSkill.__call__` now delegates any non-`claude` command to the real `subprocess.run`). `ruff check .`/`ruff format --check .` clean. |
| 2026-08-27 | sonnet | **Wave 0 of docs/GENERATION_ARCHITECTURE_ANALYSIS.md §6: deleted the dead cover-letter self-review subsystem and closed the documentation gap it exposed.** Removed 369 dead lines from `hunter/apply_shared.py` — constants `_REVIEW_SYSTEM`/`_BANNED_OPENER_PATTERNS`/`_BANNED_BODY_PHRASES`/`_BANNED_CTA_PHRASES`/`_CL_WORD_MIN`/`_CL_WORD_MAX`/`_CL_BODY_PARA_MIN`/`_CL_BODY_PARA_MAX`/`_PL_IN_EN_RE`/`_EN_IN_PL_RE`/`_METRIC_RE`, ten helper functions (`_opener_banlist_hits` … `_translate_cover_letter_pl`), and the `_cover_letter_review`/`_cover_letter_review_loop` pair — the subsystem was retired from both pipelines on 2026-06-03 (`8bd86ee`, PR #70) but never deleted, and stayed documented in CLAUDE.md as an active Pipeline Flow step (removed here). Trap avoided: `_PL_IN_EN_RE` in `apply_shared.py` is unrelated to the still-live `_PL_IN_EN_RESUME_RE` in `hunter/resume_sanitizer.py` — a plain grep for `_PL_IN_EN_RE` matches both as a substring; only the `apply_shared.py` symbol was dead. Updated the two dead re-exports in `apply_agent.py`, the one live caller (`tools/regen_covers_v2_last3.py` — removed only the `_cover_letter_review` call + import, the tool's own job is unaffected), and deleted `tests/test_cover_letter_banlist.py` (129 lines/18 tests) plus the matching dead tests and re-export checks in `tests/test_apply_shared.py` — all of it exercised the dead code THROUGH `apply_agent`, so it looked integration-shaped and stayed green regardless of whether the subsystem ran at all. Fixed `docs/PUBLIC_RELEASE_CHECKLIST.md` §1, whose "clean of personal data ✅" tick was wrong: `prompts/generation_rules.md`/`judge_rules.md` are tracked and carry the owner's 7-employer table with exact periods, university and course list (analysis doc §5.2) — the tick is now a documented ⚠️ pointing at wave 2. Widened `tests/test_handoff_readiness.py`'s scan from `*.py` only to also cover `prompts/*.md` + `.claude/commands/*.md` — confirmed this is a real finding, not a hypothetical, by temporarily emptying the new allowlist and watching it fail on exactly `prompts/generation_rules.md`/`judge_rules.md` (owner employer + owner university patterns) and nothing else; shipped with an explicit, path-scoped `LEGACY_PERSONAL_DATA_ALLOWLIST` (must shrink to empty as wave 2 lands) plus a new regression test (`test_legacy_allowlist_does_not_hide_new_offender`) proving the allowlist is scoped to those two exact files and does not swallow a new offender file in the same directories. Full suite 2749 green, ruff check + format clean. Wave 0.5 (aligning `apply_cli.py` with `apply_api.py` on four of the five missing quality stages from the analysis doc's §3) is a separate PR — the fifth, `_ats_check_loop`, is an explicit owner decision, not a mechanical port, and was left alone per instructions. |
| 2026-08-26 | opus | **Read-only architecture analysis of the generation pipeline** (owner: decompose the pieces, maybe refactor, make generation more configurable). Wrote `docs/GENERATION_ARCHITECTURE_ANALYSIS.md`; no production code touched. **(1)** The triplication grew instead of shrinking: `docs/quality/05` inventoried apply_api 1039 / apply_cli 776 / dual_apply 516 / apply_shared 1879 — today 1204 / 953 / 593 / 2211. **(2)** A stage-by-stage matrix shows the API/CLI difference is not 'one generation stage' as plan 05 assumed, but one generation stage plus FIVE missing quality stages on the CLI branch — `content_qa.run_qa`, `is_bogus_company`, `_strip_compliance_claims`, `_ats_check_loop` and the pre-LLM `is_react_only`/`is_backend_only` checks — i.e. ~a quarter of applications (14 of 60 recent runs go through `main_cli`) are generated without any of them. **(3)** ~394 lines of `apply_shared.py` (18%) are dead: the cover-letter self-review subsystem, removed from the pipelines 2026-06-03 (`8bd86ee`, PR #70) and never deleted — no caller anywhere, yet still documented in CLAUDE.md as Pipeline Flow Step 5 and covered by 18 tests that call it THROUGH `apply_agent`, so they look integration-shaped and stay green regardless. **(4)** The configurability blocker is `prompts/generation_rules.md`, which is TRACKED and carries 41 lines of the owner's personal facts (seven-employer table with periods, per-employer backend rules, university, courses, language set); both pipelines read it as their single source of rules, so neither a second user nor the owner can change that history without editing a tracked file — while the same facts already live in `candidate.yaml`. Contradicts the 'personal facts only through hunter/candidate.py' rule and the clean-tree tick in `PUBLIC_RELEASE_CHECKLIST`; `test_handoff_readiness.py` misses it because it scans `*.py` only. **(5)** Multi-user is identity-only: `users.user_env()` injects three variables, so every generation behaviour knob (`JUDGE_MODE`, `ATS_VERDICT_*`, `PRESCREEN_MODE`, `CV_GDPR_CLAUSE`, tracks, LLM profile) is process-global — and those are exactly the knobs the owner has been turning by hand. **Proposal:** four waves — W0 delete the dead subsystem, fix the two stale docs, widen the readiness gate to `prompts/*.md` + `.claude/commands/*.md` (it must fail today, that is the confirmation); W1 split `apply_shared.py` into `hunter/pipeline/*` as pure moves behind a re-export shim (also makes plan 05's PR-final-3 unnecessary); W2 render the employer block into the prompt from `candidate.yaml` the way `verdict_refine.py:60-67` already does, plus an optional per-user `generation_rules.local.md`; W3 a `generation.yaml` profile built as a literal copy of `filter_profile.py` (resolved next to `candidate.yaml`, so per-user is free), excluding the calibrated repost-gate thresholds; W4 plan 05 itself — whose blocking precondition is now satisfied (both pipelines gained golden E2E on 2026-08-24), but whose PR-0 must consume the five gaps above as explicit decisions BEFORE any stage moves. No pipeline DSL and no new LLM layer (both standing decisions). Open questions parked for the deploy host — this worktree's tracker.db is the empty fixture. **Owner decision 2026-08-27, folded back into the document after the first draft:** production runs through the CLI **deliberately** (an expensive subscription tier was bought and covers it), and CLI and API must behave **identically**. That retires §7's main open question — the `main_cli` share is a policy, not an observable — and reclassifies the five missing stages from 'a defect on ~a quarter of runs' to 'five checks absent from the PRIMARY path'. The document gained a **wave 0.5**: four of the five port mechanically into two insertion points `main_cli` already has (the gate block before `claude -p`, where `job_text` is in scope, and the scrub block at ~line 516 where `_scrub_fixes` already drives a content rewrite + re-render), and the `is_react_only`/`is_backend_only` pair is the one that also SAVES money by aborting before generation. The fifth, `_ats_check_loop`, is explicitly NOT a port: in API mode it runs before `generate_docs`, but in CLI mode the documents already exist, so after the render it degenerates into the rewrite → re-render the verdict-refine loop already performs — stacking them runs straight into the §5.7 keyword-injection-vs-judge conflict. Left as an owner decision. Wave 0.5 knowingly writes a SIXTH hand copy of stages that already exist twice, i.e. it worsens the disease waves 1 and 4 cure; the trade is accepted because a wrong CV mailed to an employer is irreversible and ~40 lines of `apply_cli.py` are not. |
