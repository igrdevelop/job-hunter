...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-12 | opus | **Two off-stack auto-applies: JustJoin's renamed skills key + a Unity role the gate called SOFT** (owner: "проверь, почему были обработаны" — an ITFS "Intermediate Frontend Developer" (React) and an EduTechPartner "Senior Software Engineer - Unity - Frontend" both got a full CV on 2026-08-12). **Diagnosis first, reproduced offline on the live postings:** both returned `classify_job → None` and `assess_job_text → []` — nothing was bypassed, prod logs show plain `[auto-apply] OK` (no `--force`, no paste), and `active_tracks()` in prod is `{angular}`, so the React track was NOT the explanation. **ITFS root cause:** the JustJoin candidate API now returns `skills: null` and puts the stack under `requiredSkills`/`niceToHaveSkills`, while `filters._is_react_without_angular` read only `raw["skills"]` — the React-without-Angular check was blind for EVERY JustJoin job, judging on the title alone (this job's required skills: TypeScript/React/Next.js, no Angular; the title says neither). Proven by injecting `skills=requiredSkills` into the live payload: `classify → react_no_angular`. The SAME key was missed in `justjoin.fetch_text`, so `job_posting.txt` lost its whole stack section — which is why the apply-time pre-LLM React screen (needs ≥3 "react" mentions) saw only 2 and also passed. Fixed both; the ITFS posting now renders 3 mentions and that second line of defence fires too. **Unity root cause:** `_GAME_ENGINE_RE` matched only "Unity 3D"/"Unity Engine", and the posting says "Unity and C#", "Unity WebGL", "Unity Test Framework" — 11 bare mentions, zero matches; even a match was SOFT (warn-only, by the 2026-07-13 decision), and `foreign_stack_no_angular` deliberately excludes C# for exactly these roles, so nothing could stop it. Added `_unity_engine_match`: bare `unity` counts as the engine only when a Unity-ecosystem token (C#/WebGL/prefab/GameObject/MonoBehaviour/IL2CPP/Addressables/shader/Unity Test Framework/game/3D) sits within 100 chars — the English word ("the team works in unity") stays clean, including when an unrelated "3D" appears far away in the same posting. **Severity raised SOFT → HARD** (owner decision, reverses 2026-07-13): an engine-first posting with neither Angular nor React anywhere leaves nothing to apply with, so it is a $0 skip; a mention under a nice-to-have heading still degrades to SOFT via the existing `_is_optional_context` guard. Live re-check: the Unity posting now yields `stack_mismatch_game_engine/hard`, the ITFS listing `react_no_angular`. 12 new tests (`tests/test_justjoin_required_skills.py` ×6 incl. the legacy flat-key path, doomed-gate game-engine hard/nice-to-have/false-positive cases); mutation-verified ×4 (revert the skills keys → 2 fail; severity back to soft → 6 fail; drop bare-Unity detection → 2 fail; drop the 100-char window → the English-word tests fail). Full suite 2556 green, ruff clean. **NOT fixed (owner deferred):** `_matches_location` accepts any location containing "remote", so findmyremote's "Remote · BR" country restriction is invisible — 2 of the 8 findmyremote jobs passing filters right now are Brazil-only; and "Intermediate"/mid levels are still not in `exclude_levels`. |
| 2026-08-12 | opus | **Scout payload v2 — accept a REAL LinkedIn Jobs url from the scout's new jobs track** (first half of the two-repo change behind the scout repo's `docs/JOBS_TRACK_PLAN.md`; owner picked "bot first, then scout"). **Why the track exists at all — measured, not assumed:** the guest search endpoint IGNORES `f_WT` (workplace type), so "remote OR Wrocław OR hybrid" is inexpressible without a session; the AUTHENTICATED search DOES honour it — LinkedIn's own counter reads 412–414 results with `f_WT=2,3` versus 489 without, with a clean control (identical URL twice → identical first-25 id sets, counter drift ~2/minute). That single fact is the whole justification for a scout-side jobs track; everything here is the receiving end. **Bot-side changes:** `MAX_SUPPORTED_PAYLOAD_VERSION` 1→2; v2 adds `kind` (absent ⇒ `post`, so every v1 payload in flight is untouched) with per-kind required fields (`_REQUIRED_FIELD_BY_KIND`: post → `body`, job → `url`). The version gate now runs BEFORE field validation — until now the `body` check ran first, so a payload from a newer scout carrying different fields would have been dropped mutely and the "update the bot" reply this branch exists for could never have fired. `linkedin_scout_relay._record_to_job` splits into `_post_record_to_job` (unchanged) and `_job_record_to_job`: keeps the REAL canonical url as `job.url` (dedup against LinkedInSource's own finds, expired-check and FAIL retries all key on it; apply fetches the description via `fetch_job_text(url, use_session=True)`), and deliberately sets NO `post_text` (its presence reroutes apply through the paste flow and skips the fetch) and NO `permalink` (it would equal `url`, which `Job.telegram_text()` already renders — the card would show the same link twice). Here `LinkedInSource` claiming the url in `_fetch_roster()` is the DESIRED routing, the opposite of the synthetic-url collision reasoning that governs posts. New `_job_location()` folds `workplace_type` into `job.location` — the central whitelist reads that one string, so a genuinely remote posting tagged "Warsaw, Poland" would otherwise be filtered out (same problem `text_utils.ensure_remote_token` solves for the JSON sources). `search()` drops a job record with an empty url instead of admitting a Job whose url normalizes to the same dedup key as every other such row. Golden fixture `tests/fixtures/scout_payload_v2_job.json`, byte-shared with the scout repo. 14 new tests (9 relay + 5 command); the pre-existing "unsupported version" test moved v2→v3. Mutation-verified ×2: breaking the kind dispatch fails 7 job-kind tests, and restoring the old version-check ordering fails exactly the ordering test (log confirms the kind check rejected the payload before the version reply could fire). **Deploy ordering is load-bearing and documented in CLAUDE.md: this repo must be live BEFORE the scout starts sending v2** — the scout marks a candidate seen after a successful SEND, and a send to a bot that then rejects the version still counts as sent, so a scout running ahead burns every found vacancy into its own seen store with nothing queued here. Also corrected a stale CLAUDE.md line that still documented the synthetic dedup url as `linkedin.com/scout-posts/…`, the legacy value replaced by `linkedin-scout.internal/posts/…` (the old one collided with `LinkedInSource.matches_url`). |
| 2026-08-12 | opus | **LinkedIn guest search read ONE page of ten per keyword** (owner hunch: "в боте мы ловим не все вакансии", raised while scoping a LinkedIn Jobs track for the standalone scout repo — see that repo's `docs/JOBS_TRACK_PLAN.md`). Measured live from the owner's desktop before writing any code, control run first (3 identical requests → identical id sets, so the endpoint does not rotate results and every parameter delta below is real). **Root cause:** the guest endpoint returns **10** cards per call, not 25. `_search_keyword` stepped `start` by `RESULTS_PER_PAGE = 25` and broke out when a page returned fewer than 25 rows — `10 < 25` on the very first call, every time. Reproduced the bot's own loop directly rather than inferring it: 10 postings where honest pagination over the same query/window yields **69**; across all probed variants 410 unique postings existed and the bot could see 7 of them. Sample titles it never reached: "Senior Angular Developer — Wrocław", "Fullstack Developer Angular & Java — Wrocław", "Starszy Programista Frontend (Angular) — Warsaw". **Two hypotheses DISPROVEN by the same measurement** (both had been written into the scout plan as real gaps): `f_E=3,4` and `f_WT` are silently IGNORED by this endpoint — byte-identical id sets with and without either — so `f_E` is deleted (it filtered nothing) and `f_WT` deliberately NOT added, with a comment saying why; the remote/hybrid/Wrocław preference cannot be expressed here at all and stays `hunter/filters.py`'s job. `f_TPR` IS honoured (24h vs 7d share only 14 of 57/69 ids) and is widened to 7 days via the new `LINKEDIN_TPR` knob — the 7-day set is 49% angular-in-title vs 11% for 24h, and URL dedup makes the wider window free. **Fix:** walk `start` on a `PAGE_STEP=10` grid up to `MAX_PAGES_PER_KEYWORD=10`, stop on an EMPTY page only — never on a short one, since the last populated page is routinely short (measured …10,10,9 then 0) and that exact assumption was the bug; per-keyword id dedup + a 1.5s inter-page pause. 10 new tests (`tests/test_linkedin_source_pagination.py`); mutation-verified by restoring stop-on-short, which failed 3 tests for the right reason (truncated walk) while the 7 parameter tests stayed green. Full suite 2536 passed, 3 pre-existing failures in `test_cost_writer`/`test_verdict_writer` — genuinely unrelated to this work and untouched here. **Correction (2026-08-13, PR #208):** the cause named in this entry, "unrelated Sheets-batching", was wrong, and "confirmed identical on a clean checkout" over-generalized. The writers were never broken. The tests' own `_seed_db` helpers called the bare `db.init_db(db_path)`, and `init_db` auto-migrates every row of the resolved `tracker.xlsx` when the target DB file does not exist yet — so each temp DB was silently seeded with ~304 real rows, inflating `skipped_no_row` to 305. Stashing local changes does not remove the *untracked* `tracker.xlsx`, which is why the re-run looked identical and the failure looked universal; it is environment-dependent, and CI plus any fresh clone are green because `tracker.xlsx` is gitignored. **Ops caveat:** LinkedIn's per-hunt yield jumps from ~10 to potentially hundreds; the central filters cut most of it and `MAX_JOBS_PER_RUN=40` caps auto-apply, but the first prod hunt after this deploys is worth watching for spend. |
| 2026-08-12 | opus | **Handoff readiness: owner PII out of shared code, and a CI gate so it stays out** (owner: "давай еще раз проверим, готов ли мой репо, чтобы дать его другу" — the THIRD such audit in three weeks, each finding something new). **Root cause of the recurrence, not just the findings:** nothing regressed between passes. `hunter/candidate.py`'s documented rule was that every `candidate.get(dotpath, default)` fallback must reproduce the owner's original hardcoded value — so every feature touching identity/location/employment LEGITIMATELY added one more line of his personal data (multi-user B3 #192, filters.yaml #195 and the scout split #200 all landed between passes). A manual audit cannot beat a continuously-regenerating source; the audit is a test now. **Live prod verification FIRST (read-only SSH), because the naive fix would have broken the owner:** his LOCAL checkout has no `candidate/candidate.yaml` at all — local generation ran entirely on those hardcoded defaults, so removing them blind kills his own flow. Prod is the opposite: `/app/users/{id}/candidate/candidate.yaml` supplies all 19 dotpaths with real values (`real_companies` is a superset of lang_guard's hardcoded token list), making the neutralization a verified no-op there. Prod yaml was copied down to both local checkouts before any code change. **Changes:** identity is now GATED, not defaulted — `candidate.require_identity()` (REQUIRED_IDENTITY_FIELDS = full_name/contact/cv_filename_prefix) raises `CandidateIdentityMissing` at the top of `generate_docs.main()`, before any file IO, exit 1; an aborted apply is retryable, a CV mailed under the wrong name is not. Placeholders replace the owner's name/aka/phone/email/LinkedIn in `candidate.py`+`generate_docs.py`; employers/profile_titles/school_keyword default to EMPTY in verdict_refine/content_qa/apply_api/lang_guard, and their consumers self-skip as "unmeasured" rather than flagging every real role as unknown (the stretch prompt drops its weaving paragraph and protects EVERY employer when no flexible one is configured). Employer names also genericized in comments/docstrings (filters calibration notes, resume_sanitizer examples) so the CI scan needs no exemption list. **Plus items 4-6 of the audit:** `.env.example` gained `SOFFICE_PATH` + the three provider-specific key vars (SETUP told users to set them; they were absent), README's stale "21 boards / 1700+ tests / ~14 s" → 25 / 2500+ / ~10 min, real VPS IP redacted from AGENT_LOG, and SETUP_NEW_USER's false promise that skipping candidate setup "runs with example data" replaced with what actually happens. **New:** `tests/test_handoff_readiness.py` (PII literals in `hunter/`+entry scripts; every SETUP-documented env var present in `.env.example`; every `candidate.get` dotpath documented in candidate.yaml.example) and `tests/test_candidate_identity_gate.py` (8 cases incl. a wiring test that passes a nonexistent content.json — exiting with the identity error instead of FileNotFoundError proves the gate runs before file IO). conftest's `_owner_candidate_data` fixture extended to pin content_qa's now-empty whitelists so QA tests keep exercising real comparison logic on CI. Mutation-verified ×2 (restoring the owner name literal → readiness scan fails; removing the gate call → wiring test fails with FileNotFoundError). **Full suite 2538 green in BOTH modes** — with candidate.yaml present and with `CANDIDATE_YAML_PATH` pointed at a nonexistent file (fresh-clone/CI simulation); ruff clean. **NOT done (owner's call, item 1):** personal data still exists in git HISTORY — `prompts/candidate_profile.md` (real phone/email), `prompts/candidate/interviews *.txt` (8.7KB + 5.8KB of private interview notes), base CVs — removed from the tree in a3781f6 but present in all 946 earlier commits. No `.env`/token/tracker was EVER committed (verified). Scrub recipe: docs/PUBLIC_RELEASE_CHECKLIST.md §3. |
| 2026-08-12 | opus | **Gmail digest titles — one subject, N vacancies** (owner report: two off-stack rows auto-applied on 2026-08-12 — "Software Engineer" (LanceSoft Europe, a React/**Java** 12-month contract in wealth management) and "Full stack / Front-end developer - **Scala / Java**" (Luxoft)). **Diagnosis (live VPS, read-only):** both arrived from `gmail_linkedin`, not the LinkedIn scraper. `_jobs_from_urls` labelled EVERY url of an alert email with the email SUBJECT — `_title_from_url` returns "" for LinkedIn's bare-id slug — so the LanceSoft url reached the filters titled "Front-End Developer (Agentic AI) at Comarch" (6 urls in that email) and the Luxoft one "…LEMISOFT - Programista Frontend (Angular)…" (3 urls). Both sail past `title_keywords`/`exclude_patterns`; `linkedin.com` sits in `GMAIL_ENRICH_SKIP_HOSTS` (prod uses the default) so the stub is never repaired; and `apply_service` passes that same wrong title into the doomed gate as `--title`, where the title rules validated a string belonging to another vacancy. Body rules could not save it: React in the text vetoes `foreign_stack_no_angular`, and Java/Scala are not in that list at all. Replaying the REAL titles through `classify_job`: `title_kw` (LanceSoft) and `exclude_pattern` \bjava\b (Luxoft) — the listing filter would have rejected both. **Second, independent damage from the same stub:** `dedup_key("[linkedin]", subject)` is identical for every job of one email, so the hunt silently dropped all but the FIRST as "Dup company+title" — 25 such discards in a single 19h log window, i.e. real vacancies lost, and the survivor is whichever url comes first in the markup, not the one the subject describes. **Three fixes:** (1) `gmail_parsers._linkedin_cards` reads each alert card's own title + company straight from the email HTML (an `<a>` to /jobs/view/&lt;id&gt; whose text is the title, followed by "&lt;Company&gt; · &lt;City&gt; (Remote|Hybrid)"), verified live against 8 real alerts — 31/31 cards parsed, incl. `4451158730 → "Software Engineer" @ "LanceSoft Europe"`; the subject remains a fallback only. (2) The doomed gate stops trusting the handed title blindly — `_titles_to_check`/`_titles_agree` additionally guess the posting's OWN header line and run the title rules against both when they describe different jobs (Luxoft → HARD `title_exclude_pattern` → $0 skip even with the wrong title); `_dedup_findings` keeps one line per rule. This supersedes the old contract "a guessed title never runs when the real title is known", whose test is rewritten into an agreement case + a disagreement regression. (3) `_stub_company` tags the placeholder per-URL (`[linkedin]#<id>`) when the subject fallback fires, so a digest can never collapse onto one dedup key again. **Replay of the real Comarch digest after the fix:** 3 genuine frontend roles now pass (before: one, mislabelled), LanceSoft + Balyasny → `title_kw`, NATEK "Fullstack Developer (Angular/Java)" → `exclude_pattern`. 8 new tests (`tests/test_gmail_linkedin_cards.py`) + 3 in `test_doomed_gate.py`; mutation-verified ×3 (card-title extraction, the gate's disagreement branch, stub uniqueness). Full suite 2529 green, ruff clean. **Deliberately NOT changed:** the card's location parses just as cleanly ("Gdańsk (Remote)", "Katowice (Hybrid)") but is still dropped on the floor — today every gmail job has `location=""` and therefore bypasses the location whitelist entirely; feeding it in would start rejecting jobs that currently pass, which is an owner call, not a bug fix. |
