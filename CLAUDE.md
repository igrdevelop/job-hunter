...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-27 | sonnet | **Wave 1 PR C of docs/GENERATION_ARCHITECTURE_ANALYSIS.md §6: moved the remaining gates, the language enforce-gate, and post-generation abort out of `hunter/apply_shared.py` — the file is now a pure re-export shim.** Third and final PR decomposing `apply_shared.py` (1843 → 88 lines across the three PRs, zero function/class definitions left in the shim itself). Moves: `hunter/pipeline/gates.py` (`is_react_only_job_text`, `is_backend_only_job_text`, `_already_processed`, `run_doomed_gate`, `stack_gate_allows_manual`, `run_prescreen`), `hunter/pipeline/lang.py` (`enforce_language_separation`, `ensure_pl_resume`, `build_pl_skip_instruction`, `_translate_resume`, `_translate_plain`), `hunter/pipeline/abort.py` (`abort_after_generation`, `_write_abort_skip_row`, `_handle_jobleads_fetch_blocked`). Three more instances of the PR-A/B shim-compatibility indirection, each proven necessary by an existing test rather than applied speculatively: **(1)** every gates.py function that calls Telegram (`run_doomed_gate` ×2 call sites, `stack_gate_allows_manual`, `run_prescreen` ×3) and both abort.py functions (`abort_after_generation`, `_handle_jobleads_fetch_blocked` ×3) re-read `notify` from `hunter.apply_shared` at call time — `test_doomed_gate_wiring.py`, `test_manual_stack_gate.py`, `test_prescreen.py`, `test_apply_cli_abort.py`, `test_abort_identity.py` and the golden E2E tests all monkeypatch `apply_shared.notify` while calling these functions directly (not through their normal callers), so the function that owns the Telegram side-effect has to be the one re-resolving it, regardless of which physical file now defines it. **(2)** `_translate_resume`/`_translate_plain` (lang.py) re-read `_translate_p` from `hunter.apply_shared` — `test_lang_enforce_gate.py::test_no_api_key_no_repair` patches `apply_shared._translate_p` then calls `apply_shared.enforce_language_separation`, which reaches `_translate_resume`/`_translate_plain` internally; `_translate_p` itself lives in the already-moved `profiles.py`, a different file from both callers. **(3)** `ensure_pl_resume` (lang.py) re-reads `_translate_resume` from `hunter.apply_shared` for the same reason — `test_pl_resume_mirror.py` patches `apply_shared._translate_resume` (via `monkeypatch.setattr(apply_shared, ...)`, qualified module access) and calls `ensure_pl_resume` directly; both functions ended up in the SAME file (`lang.py`) but that doesn't help, since the patch lands on `hunter.apply_shared`'s own `__dict__` entry, not `lang.py`'s. Every one of these edges was found BEFORE moving any code, by grepping the whole test suite for every `hunter.apply_shared.<name>` mutation style (`monkeypatch.setattr` both string-path and object-attr forms, `unittest.mock.patch`, and raw attribute assignment) and cross-referencing against the internal call graph inside the original `apply_shared.py` — not discovered by trial and error against failing tests. Full suite 2804 green (unchanged — no new tests needed beyond PR A's shim pins), both golden E2E suites pass, `ruff check`/`format` clean. `apply_shared.py`: 916 → 88 lines; final `hunter/pipeline/` package: 10 files, ~2100 lines total (vs. the original 1843-line monolith — the growth is re-export headers + doubled module docstrings, not duplicated logic). |
