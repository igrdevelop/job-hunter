...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-27 | sonnet | **Fixed a vacancy stuck bouncing PENDING↔IN_PROGRESS forever — the company+title dedup gate's own SKIP write was silently no-op'ing.** `_is_known_terminal(url, company, title)` — the idempotency guard every terminal-write function (`add_skipped`/`add_failed`/`add_expired`/`add_react_skipped`) checks before writing — matched company+title GLOBALLY across the whole table, not scoped to the url being written. A repost (AVENGA "Senior Angular Developer" on nofluffjobs, new URL) correctly matched an unrelated April application by company+title at the Step 4.55 dedup gate, which called `add_skipped` for the new url — but the guard saw the April row's terminal status and silently no-op'd, never clearing the new url's own IN_PROGRESS/PENDING placeholder, so it bounced back to the queue every ~60min stale-claim reset, burning a full CLI generation attempt each time, forever. Measured before generalizing further: the raw listing's staffing-agency suffix ("Agencja Pracy, nr KRAZ: 8448", not stripped by `_strip_legal_suffixes`, hence not caught by the cheap hunt-time dedup either) appears on exactly ONE company in the live 1333-row tracker — not worth a dedicated regex. Fix: the four terminal-write functions now call `_is_known_terminal(url)` URL-ONLY (`add_manual_jobleads_pending` already did this and was the template); the company+title branch is reserved for the one read-only caller that needs it (`apply_worker._resolve_outcome`). 4 new mutation-verified regression tests in `tests/test_tracker_pending.py`. Full suite 2762 green, ruff clean. Live prod row self-heals on next deploy. |
| 2026-08-27 | sonnet | **Diagnosed a 36h "no new vacancies" gap and stopped the LLM-outage alert from burying itself in its own repeats.** Live SSH investigation on the deploy host (owner report): hunting was fine (13 jobs queued PENDING), but every apply since 2026-08-26 00:40 hit exit 46 — `logs/apply_stdout/*AVENGA*.log` showed a genuine double failure, the Anthropic API balance was drained AND the Claude-CLI subscription (M4b fallback) OAuth session had independently expired (its ~30-day refresh-token lifetime ran out; last login was 2026-07-29) — normally the CLI fallback silently absorbs an API outage, but both were down at once. Fixed live via `/login`. Follow-up: `llm_outage.arm_pause()` now returns `(until, is_fresh)` (new DB key `llm_outage_streak_since`) — a long outage re-probes and re-arms roughly hourly, and the old "one alert per arm" design would have sent ~36 near-identical Telegram messages for this one incident. `apply_worker._resolve_outcome` (the live prod path) and `main.py`'s two batch-loop sites now alert loudly (with concrete "check balance AND CLI login" guidance) only when `is_fresh`, log-only on a streak continuation; `clear_pause()` clears the streak too and is now called on every real success so a later, unrelated outage alerts fresh again. `tests/conftest.py`'s `tracker_db` fixture gained a matching patch of `hunter.llm_profiles._get_db_path` to keep the new success-path `clear_pause()` calls out of the real repo tracker.db during tests. Full suite 2758 green, ruff clean. Full diagnosis in docs/AGENT_LOG.md. |
