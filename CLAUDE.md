...
| `PRESCREEN_MODE` | `report` | `report` (log only) → `warn` (+Telegram) → `skip` (SKIP row, no generation). Ships at `report`: the calibration is offline evidence, and a week of live verdicts beside real outcomes is what earns the flip (owner decision 2026-08-24). |
