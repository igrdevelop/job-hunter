...
| `LLM_OUTAGE_FALLBACK_CLI` | `false` | M4: on an API account outage, retry the vacancy ONCE through the Claude CLI (Pro subscription) before reporting exit 46. When ON, the CLI is reserved as the fallback (the "CLI detected → try CLI first" auto-preference in `apply_agent.main()` is skipped, so the paid API stays primary). Repo side is in place (CLI in the Dockerfile, `CLAUDE_CONFIG_DIR`/`IS_SANDBOX` envs, `./.claude-cli:/root/.claude` compose volume, `.claude/commands/` re-included in the image for the `/apply` slash command); to activate: one-time `docker compose exec -it job-hunter claude` OAuth login (owner decision 2026-07-18 — puts the personal subscription token on the deploy host, gitignored `./.claude-cli/`) + this flag in `.env`. See the plan's M4 checklist. |
