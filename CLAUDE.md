...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-07 | fable | **Multi-user B3 — runtime, owner-only hunting scope** (branch feat/multi-user-b3, docs/MULTI_USER_UPDATE.md; scope decision same day: hunting stays owner-only in B3, non-owner users get manual tailoring only — hunt fan-out + per-user search specs moved to a new Phase B3.5). One commit per item. **B3.1** `/link CODE` + `/unlink` (hunter/commands/link.py; hunter/users.py — single-use codes from `telegram_link_codes`, one-to-one `telegram_links`, re-linking moves the row). **B3.2** registry: `resolve_user`/`resolve_chat`/`user_paths` (new config `USERS_ROOT`, users/{uid}/{candidate,Applications,templates})/`list_active_users` (non-owner force-excluded until B3.5). **B3.3** hunter/bot/auth.py `require_user`/`require_owner` around EVERY handler: /start /link /unlink open, operational commands owner-only, URL paste any linked user, buttons owner-only; the admin chat (TELEGRAM_CHAT_ID) counts as owner without a link row (pre-seed deploy can't brick the bot) — closes the pre-B3 no-auth hole. **B3.4** candidate.py cache keyed per resolved path (maxsize=32); `load()`/`get()` take optional `path`. **B3.5b** manual tailoring: `config.current_user_id()` (JOB_HUNTER_USER_ID env wins over DEFAULT_USER_ID; tracker._uid delegates), `users.user_env()` injects CANDIDATE_YAML_PATH/APPLICATIONS_DIR/JOB_HUNTER_USER_ID/TELEGRAM_CHAT_ID into the apply subprocess (config reads env at import → zero pipeline changes), `run_apply_agent_for_url(extra_env=)`, non-owner runs skip Sheets/Drive delivery, `_tg_notify(chat_id=)`; same URL for two users = two rows. **B3.7** `tracks_enabled`/`dual_*` move to `user_settings` per current user (legacy config-row fallback); `llm_outage_until`/`active_llm_profile` stay global. **B3.8** user scoping on `read_all_tracker_rows`/`iter_unsent_rows` (Sheets/Drive/expiry feeds) + `get_recent_applied_for_repost` (repost gate can never donate another user's CV). ~70 tests. **Ops at deploy:** seed owner telegram_links row (SQL in the plan doc, item 9). Full detail in docs/AGENT_LOG.md. |
