...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-08 | fable | **SKIP/FAIL rows stamped Sent='—'** (branch claude/results-table-fail-skip-rows-b21ce6; owner asked why 8 Sheet rows showed SKIP/FAIL and to dash their Sent so they leave his "Sent is empty" send-queue filter). Investigation on the live VPS: Primer FAIL = Ashby fetch returns 64 chars; Renesas FAIL was later retried successfully (Sheet row `aeddc5d6` is a stale duplicate — delete by hand); the 6 SKIPs are doomed-gate HARD skips (German required ×2, London onsite, Atos .NET title, 2× LinkedIn text-at-run-time). Fix: `add_skipped`/`add_failed` write `sent='—'` at insert; `db.py` idempotent backfill stamps old blank-Sent SKIP/FAIL rows (+`sheets_dirty=1` so resync pushes); pull conflict matrix protects ALL bot markers (EXPIRED + dashes) from a blank Sheets cell, not just EXPIRED. Accepted side effect: '—' on SKIP = react-skip marker → plain re-paste refused, `/force` regenerates; FAIL retries unaffected (key on ats_status only, regression-tested). 7 new tests; suite 2597 green. Live DB+Sheet fixed same session. Full detail in docs/AGENT_LOG.md. |
