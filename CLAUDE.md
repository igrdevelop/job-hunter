...
hunter/
  ...
  metrics.py                Per-run metrics log for the apply pipeline (docs/
                            ...
                            must exclude those from timing). KNOWN GAPS the snapshot
                            tool measures (PIPELINE_VIZ M0): events are written only at
                            the END of a stage (no `start`), the refine loop writes no
                            per-round event, and a run killed by the worker's timeout
                            never gets `finish_run` (leaks `finished_at IS NULL`). No
