...
hunter/
  ...
  repost_gate.py            Same-vacancy re-post gate (Step 1.5g, $0): TF-IDF text match
                            ...
                            shadow). Calibrated thresholds 0.97/0.90/0.85, min 1500 chars.
...
3c. **Re-post gate** (`hunter.repost_gate.run_repost_gate`, Step 1.5g in BOTH
   ...
   near-identical words — live at 0.85-0.90): sim>=0.97 → re-post regardless
...
| Date | Agent | Work |
|------|-------|------|
| 2026-07-31 | sonnet | **In-flight apply concurrency guard** (worktree branch claude/billennium-duplicate-check-67d89e; owner report: a Billennium "Mid Frontend Developer" LinkedIn posting on 2026-07-30 got two full generations — `Billennium/` ($0.3074, 9 LLM calls) and `Billennium_2/` ($0.3355, 10 LLM calls) — ~$0.64 combined for one vacancy). Root cause: `add_applied()`'s URL dedup (`hunter/tracker.py`) only fires at the END of a successful run, when the tracker row is written — nothing stopped a second entry point from starting the SAME URL's full LLM pipeline while the first was still generating. Here the auto-hunt loop found the posting the same moment the owner pasted it manually; both won a folder (`compute_output_folder`'s `_2` suffix), only the first to finish could write the tracker row (`add_applied` returned `False` for the second — `Billennium_2` was a silent orphan: no tracker row, never uploaded to Drive, and duplicated into Sheets once the first row's own late mirror also double-fired). Fix: `hunter/bot/state.py`'s existing `_active_apply_urls` dict (previously `/status`-display-only) now doubles as a concurrency guard via two new functions, `try_mark_apply_active(url)`/`mark_apply_done(url)`, keyed by `tracker.normalize_url` (no lock needed — asyncio is single-threaded, check+set never yields). All three entry points that can start generation now claim the URL first and release it in a `finally`: `hunter.main._run_apply_agent` (hunt/retry loops — new `duplicate` outcome, handled like `rate_limited` in `_auto_apply_all`/`_retry_failed`: no FAIL row, `fail_count` untouched, job just returns on the next hunt/retry slot), `hunter.bot.apply_runner._run_apply_agent` (manual paste + Apply-button — notifies "⏭ Already generating" and returns without spawning a subprocess), and `_run_linkedin_batch` (per-job guard inside its existing sequential loop, for symmetry with the other two paths). This is a second, tighter layer on top of `_handle_paste`'s existing tracker-based "already in tracker" check (`lookup_url`) — that one only catches a URL that ALREADY finished once; this one catches the race window while a run is still in progress. 96 tests across the affected modules green (`test_apply_service.py`, `test_hunter_apply_agent.py`, `test_hunt_queue_delivery.py`, `test_llm_outage*.py`, `test_main_auto_apply_permalink.py`, `test_main_manual_only_partition.py`, `test_notify_fallback.py`, `test_rate_limit_no_escalation.py`, `test_retry_failed_breaker.py`, `test_url_message_relay_apply.py`); ruff check/format + compileall clean. **Ops note (not done here):** the live Sheets tracker still has the 2026-07-30 duplicate Billennium row (1107/1108, both `c41f4700`) and the orphaned `Billennium_2` local folder from before this fix — a manual one-time cleanup, not something this guard retroactively fixes. |
| 2026-07-30 | sonnet | **Hunt / apply split — M3, M4, M1** (branch feat/hunt-apply-split, docs/HUNT_APPLY_SPLIT_PLAN.md + docs/HUNT_APPLY_SPLIT_PROMPT.md; M2 (N>1 parallel workers) and M0 (measurement) explicitly deferred/skipped per owner instruction). Problem: `_auto_apply_all` runs the whole per-hunt apply batch inline inside `_hunt_lock`, so a CLI-fallback-widened apply (up to `APPLY_AGENT_CLI_TIMEOUT_SEC`=2700s) blocks every other scheduled hunt slot behind it. **M3 (`cli_timeout` outcome, `hunter/services/apply_service.py`):** when a subprocess `asyncio.TimeoutError` fires AFTER `_effective_timeout` widened the caller's budget for a CLI-eligible run, that's now a distinct `ApplyOutcome` from a plain `"fail"` — infrastructure flakiness, not the vacancy's fault. `_auto_apply_all`/`_retry_failed` (`hunter/main.py`) and `bot/apply_runner._run_apply_agent` treat it like `llm_outage` (no FAIL row, no `fail_count` escalation, Telegram notify, job returns next hunt) except it does NOT stop the batch or arm any pause — purely per-job, not a global account state. **M4 (fail audit log):** new `hunter/apply_failures_log.py` — `log_apply_failure()` appends one JSON line (`{ts, url, company, title, outcome, exit_code, error, duration_sec, cli_mode}`) to `logs/apply_failures.jsonl` (RotatingFileHandler 5MBx5, `propagate=False`) for every `"fail"`/`"cli_timeout"`/`"rate_limited"` outcome from `apply_service` (`"llm_outage"` deliberately excluded — already tracked by `hunter.llm_outage`); best-effort, a logging failure never breaks an apply run. New read-only `/fails [N]` command (`hunter/commands/fails.py`, default 10/max 30). **M1 (PENDING queue + hunt/apply split, the main piece, gated by new `APPLY_QUEUE_ENABLED` default `false`):** `hunter/db.py` gained `claimed_at`/`pending_meta` columns; `hunter/tracker.py` gained `add_pending`/`claim_pending` (atomic `UPDATE…RETURNING…ORDER BY rowid` for real FIFO — `id` is a random UUID, not insertion-ordered)/`release_claim`/`reset_stale_claims`/`job_from_pending_row`/`count_pending`/`count_in_progress`/`list_pending`; every terminal writer (`add_applied`/`add_failed`/`add_skipped`/`add_react_skipped`/`add_expired`) now checks a new `_is_known_terminal()` (True only for a genuine, non-placeholder row) instead of the unchanged, still status-agnostic `is_known()`, and calls a new `_clear_own_placeholder()` first so the queue's own bookkeeping row never collides with the real outcome row; `_COOLDOWN_SKIP_STATUSES`, `iter_unsent_rows`, and `read_all_tracker_rows` all exclude PENDING/IN_PROGRESS so a queued-but-unresolved job stays invisible to cooldown checks, `expired_marker`, and Sheets/Drive sync. New `hunter/apply_worker.py::apply_worker_loop` — a background `asyncio` task (started from `telegram_bot._post_init`) that claims PENDING rows and runs them through the SAME `apply_agent.py` subprocess forever, independent of `_hunt_lock`, with the same outcome handling as the old inline batch loop plus a 3-consecutive-fail breaker (backs off 5 min, never exits — this loop has no finite batch). `hunter/main.py`'s AUTO_APPLY branch now branches on `APPLY_QUEUE_ENABLED`: on, it writes a `PENDING` row per new job and returns in seconds (fetch+filter+dedup only); off, unchanged inline behavior. New `hunter/schedules/apply_queue.py::scheduled_reset_stale_claims` (every 15 min) resets `IN_PROGRESS` rows older than new `APPLY_CLAIM_TIMEOUT_MIN` (default 60) back to `PENDING` — crashed-worker recovery. New read-only `/queue [limit]` command; `/status` shows PENDING/IN_PROGRESS counts when the flag is on. **Bug found and fixed along the way:** `apply_shared._handle_jobleads_fetch_blocked` used `lookup_url` to check "already tracked", which fired on the queue's own PENDING/IN_PROGRESS placeholder and silently swallowed the JobLeads MANUAL row — replaced with `_is_known_terminal`. Extensive new test coverage (`test_apply_failures_log.py`, `test_tracker_pending.py`, `test_apply_worker.py`, `test_main_apply_queue_wiring.py`, `test_queue_command.py`, `test_apply_queue_schedule.py`, `test_status_apply_queue.py`) plus updates to `test_db.py`. Full suite green (deselecting 3 known pre-existing `test_cost_writer.py`/`test_verdict_writer.py` isolation failures unrelated to this change — they migrate the REAL `tracker.xlsx` into their test DB, a bug predating this branch); ruff check/format clean. |
| 2026-07-30 | sonnet | **Drive SSL race — M1/M2/M3** (branch claude/gdrive-ssl-race-impl-06fe8c, docs/GDRIVE_SSL_RACE_PLAN.md). Owner-visible symptom: `[SSL] record layer failure` alerts from concurrent Drive uploads. Root cause: the post-apply delivery fallback and the scheduled backfill routinely overlap, both driving the ONE cached googleapiclient service's non-thread-safe httplib2 socket. **M1**: `upload_missing_folders` re-entrancy guard (`_backfill_running` bool, try/finally) — a concurrent call short-circuits with `skipped_busy` instead of re-walking the same folder list. **M2**: `_FOLDER_LOCK` renamed `_DRIVE_LOCK`; new `_drive_call()` helper routes EVERY Drive API call (not just folder resolution) through the shared lock, acquired per call; `build_service` now sets an explicit `httplib2.Http(timeout=GDRIVE_HTTP_TIMEOUT_SEC)`; timeout handlers call `_invalidate_service()` so an abandoned thread's poisoned socket is never reused. **M3**: new `hunter/drive_ledger.py` (content-signature `drive_uploads` table) skips re-uploading unchanged dual-apply shadow subfolders (were re-uploaded on every 30-min pass forever, ~700 pointless API calls/pass); `/gdrive_upload_missing force` bypasses it. Full details incl. mutation-verification notes in docs/AGENT_LOG.md. Full suite green (2410); ruff/compileall clean. **Ops still outstanding:** `tools/dedup_drive_folders.py` against the live Drive (plan §3, not run here). |
| 2026-07-29 | opus | **Claude Code tooling: 2 agents, 2 skills, 2 commands** (branch claude/virtual-ai-interview-efb153; docs + `.claude/` only, no production code). Filled the empty stages of the dev cycle — review, tests, release, planning. Deliberately NOT built: a generic review agent (duplicates the built-in `/code-review`), a PR-wrapper agent (`gh pr create` exists) and a "run the tests" agent (pytest + CI already gate that). **`agents/project-invariants-review.md`** — the branch diff against 11 invariants specific to THIS repo, each from a real incident: CLAUDE.md updated in the same commit, new config var in the config table, new swallow-and-continue code wrapped in `best_effort()`, `requirements.lock` regenerated after a `pyproject.toml` edit and never hand-edited, all five source-registration points, tracker column constants vs the documented schema, English-only commit prose, protected files not staged (hard stop), mypy baseline 223 not grown (CI is `continue-on-error`, so regressions are invisible by design), a soft "which real decision does this LLM call change?", and a work-log entry on large diffs. Finds no bugs and no style issues BY DESIGN. **`skills/mutation-verify/`** — formalises the "mutation-verified" convention already used in this log: mutate the guarded production line, run ONLY that test, require a RELEVANT failure (an `ImportError` proves nothing), restore, confirm green; refuses to run on a dirty target file since restore is `git checkout --`. **`agents/fail-forensics.md`** — why one vacancy produced no application, from tracker.db + `logs/hunter_errors.log` + the folder, against a signature table for every pipeline stage (incl. the exit-46 outage case that deliberately writes no FAIL row), then retry vs permanently-dead (`fail_count` >= MAX_FAIL_RETRIES → `/retry_reset`). Read-only — retries cost money. **`skills/cost-audit/`** — `cost_usd` totals/median/tail, bands by `ats_verdict`, funnel correlation via the existing `tools/verdict_funnel_corr.py`; aimed at the verdict-refine loop, whose rolled-back rounds also cost. **`commands/pr.md`** — fetch → branch cut from CURRENT `origin/master` (new branch, never a rebase) → ruff check + format --check + pytest → invariants pre-flight (hard stops: staged protected file, lock/pyproject mismatch) → English-only body, no Co-Authored-By → `gh pr create`. **`commands/plan-doc.md`** — `docs/<NAME>_PLAN.md` before code, after searching AGENT_LOG for an already-rejected version; M0 is always a free read-only measurement with its decision rule stated BEFORE it runs. **Verification finding:** the first drafts of the cost/forensics queries named `cost_usd`/`ats_verdict`/`fail_count` directly and died with `no such column` — the dev-checkout tracker.db is a stale 14-row fixture predating those startup migrations, and `logs/` doesn't exist in a worktree. Both now probe `PRAGMA table_info` and read defensively, degrading to "unmeasured — wrong DB copy"; live numbers need the deploy host. Full detail in docs/AGENT_LOG.md. |
