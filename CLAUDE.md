...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-07 | sonnet | **Multi-user B1+B2** (branch claude/multi-user-update-docs-17e093, docs/MULTI_USER_UPDATE.md). **B1 — schema mirror + user stamping:** `hunter/db.py` mirrors the shared-contract DDL idempotently: `user_id TEXT NOT NULL DEFAULT ''` added to `applications`, old global `idx_url_norm` dropped, new `idx_user_url_norm` UNIQUE on `(user_id, url_norm)` + `idx_user_ats`; `user_settings`, `telegram_links`, `telegram_link_codes` tables created. New `DEFAULT_USER_ID` env in `hunter/config.py`. `hunter/tracker.py`: `_uid()` helper threads `DEFAULT_USER_ID` through every dedup check (`get_known_urls`, `get_known_company_titles`, `is_known`, `_is_known_terminal`, `_clear_own_placeholder`) and every INSERT. Test `test_get_url_status_flags_ignores_fail_and_plain_skip` split into two separate-URL tests (the scenario it tested — two terminal rows for the same URL — is now impossible under the unique constraint). **B1.3 — CANDIDATE_DIR env derivation:** `apply_shared.CANDIDATE_DIR` now derives from `Path(CANDIDATE_YAML_PATH).parent` when the env var is set (falls back to `PROJECT_DIR/candidate`); `claim_judge` and `resume_sanitizer` import it from there instead of computing their own copies. **B2 — per-user settings helper:** `user_setting(user_id, key, default)` added to `hunter/config.py`; reads `user_settings` table, best-effort (returns default on any DB error). 4 tests. Not yet wired into flow control (B3). |
| 2026-08-04 | grok | **LinkedIn expired detection via Playwright session** (docs/LINKEDIN_EXPIRED_DETECTION_PLAN.md). Guest LinkedIn HTML omits "No longer accepting applications", so apply was spending ~$0.31 LLM on dead postings. Split detail fetch: `LinkedInSource.fetch_text` stays unauthenticated (html_fallback) for `/check_expired`/enrichers; new `fetch_text_with_session` + `_playwright_fetch` (sync Playwright, `storage_state`, image/font/media blocked, 15s timeout, soft fallback on failure) is used only when `fetch_job_text(url, use_session=True)`. Wired in `apply_api` + `apply_cli`; left `expired_marker`/`gmail_enricher`/`repost_gate` on the guest path. Tests for session/fallback + dispatcher `use_session` flag. |
