...
| 2026-08-31 | sonnet | **Bot-repo work item from docs/PROFILE_PAGE_TABS_WORKORDER.md: a deterministic, $0, NO-LLM "test resume" preview — new `profile_jobs.kind='preview'`.** The work order (site/api companion doc, #245) specs a fourth `/profile` tab ("what will the system actually produce from my profile") that must never spend an LLM call — there's no real vacancy to generate against. New `hunter/profile_preview.py`: `build_preview_content(profile, track)` assembles a generic no-vacancy content.json-shaped dict straight from a `hunter.profile_schema.Profile`, reusing `hunter.profile_render`'s own private track-resolution helpers (`bullets_by_track` wholesale override, `Role.tracks` visibility, variant skills override) for a real track key — but `track="core"` is a deliberate special case returning every role/bullet/skill UNFILTERED, matching `render_profile_md()`/`employers.history`'s own "core is facts, not one track's presentation" philosophy (those two also ignore `Role.tracks`/`Bullet.tracks`). `resume_en.skills` is matched against the fixed 4-category document layout by case-insensitive label — a profile category whose label doesn't match Frontend/Tools/Methodologies/Languages isn't shown, the same limitation the real LLM-driven pipeline has for any category a user's own `generation.yaml` doesn't also configure (flagged as a known simplification, not fixed here). `render_preview(profile, track, out_dir, extra_env=...)` writes `content.json` then drives `generate_docs.py` as a subprocess with `--no-tracker` (never a tracker row — a preview is not an application) and env `GENERATE_ABOUT_ME_PL=false` (generate_docs.py's about-me step is an LLM call and defaults ON even outside `--full` mode — the actual $0 guarantee lives in this one env override, not just the missing `--full` flag). `validate_track()` rejects anything that isn't a simple lowercase slug ('core' or a track key) before it's used as a path component — same wariness as `_resolve_user_relative_path`, since the track string crosses a process boundary via a shared DB row. `hunter/schedules/profile_jobs.py` gained `kind='preview'`: payload `{"profile": ..., "track": ...}`, output into a fresh dated subfolder `users/{uid}/candidate/preview/<track>/<UTC timestamp>/` (microsecond precision — history kept, never overwritten, owner decision 2026-08-31), requires the user's candidate.yaml to already exist (profile published at least once) and fails with a "publish the profile first" message otherwise rather than half-rendering under a placeholder identity. `tools/preview_profile.py` is the CLI seam (same thin-argparse-wrapper pattern as `tools/parse_resume.py`/`render_profile.py`, single-user/dev, no logic of its own). 55 new tests across `tests/test_profile_preview.py` (content assembly + track resolution, generate_docs.py subprocess boundary faked exactly like `tests/test_golden_apply_e2e.py`'s own `FakeGenerateDocsRunner` since real LibreOffice is slow/environment-dependent), `tests/test_schedules_profile_jobs.py` (job-level: dated subfolder, no-tracker row, missing-candidate.yaml error, unsafe-track rejection) and `tests/test_tools_preview_profile_cli.py` (real-subprocess CLI tests, tolerant of a missing local LibreOffice binary — asserts on content.json's resolved content instead of insisting on a `.pdf` extension). Two track-resolution rules (bullets_by_track wholesale override, Role.tracks visibility) mutation-verified by hand: each guarding line was temporarily reverted to the naive behavior, the specific test confirmed to fail for the right reason, then restored. Full suite 3083 green (was 3028 on master), ruff clean, mypy baseline confirmed unchanged at 218 (isolated `origin/master` worktree comparison). |
| 2026-08-31 | sonnet | **Work-order steps 2d and 4b of docs/RESUME_PROFILE_STORE_PLAN.md: role-level track visibility, a per-track base_cv preamble, and the `profile_jobs` render/parse queue drain.** These two steps were added to the work order AFTER M1-M4 (#238-#241) had already shipped and were tracked as an explicit gap. **Step 2d** (`hunter/profile_schema.py`, `hunter/profile_render.py`): `Role.tracks` hides a role from OTHER tracks' `base_cv_<track>.md` entirely (same shared-unless-tagged contract as `Bullet.tracks`) while `render_profile_md()` and the `employers.history` projection in `render_candidate_yaml()` deliberately keep including every role regardless of tracks — those are facts (the narrative superset, the judge's RED LINES ground truth), not a track's presentation. `Variant.notes` is a free-text per-track instruction rendered as the very first block of `base_cv_<track>.md`, before the headline — gives the react track's "never write Angular in a role title" preamble (found missing on the #239 review) a schema home. `candidate/profile.example.json` updated: the Beta Solutions role tagged angular-only, the angular variant gets a neutral `notes` string. **Step 4b** (new `hunter/profile_jobs.py`, new `hunter/schedules/profile_jobs.py`, `hunter/db.py`): the bot-side half of the cross-repo job bus whose contract is owned by `job-hunter-api/docs/RESUME_PROFILE_STORE.md` — verified against the ACTUAL merged api-repo implementation (`claude/profile-store-audit-abd6f5`, already merged to that repo's `origin/master`), not just its doc, since the two can drift; the `profile_jobs` DDL matched byte-for-byte, no discrepancy found. `hunter/db.py` mirrors the DDL idempotently (same precedent as `telegram_link_codes`). `hunter/profile_jobs.py` owns the atomic primitives: `claim_next_profile_job()` (one `UPDATE...RETURNING`, mirroring `tracker.claim_pending`), `finish_profile_job`/`fail_profile_job` (terminal done/error), `reset_stale_profile_jobs(timeout_min)` (crash recovery for a `running` row whose drain tick died). `hunter/schedules/profile_jobs.py::scheduled_profile_jobs_drain` runs every 20s (registered in `hunter/schedules/__init__.py`): `kind='render'` calls `profile_render.render_all()` into `users/{uid}/candidate/` AND writes `profile.json` itself next to the rendered files (wave-2 groundwork for a future direct-structure consumer); `kind='parse'` resolves the payload — a path relative to `users/{uid}/` supplied by another process over a shared DB row — through `_resolve_user_relative_path()`, which rejects an absolute path (the `Path.__truediv__` absolute-overrides-left-operand footgun) or any `..` escape via `Path.is_relative_to()` BEFORE touching the filesystem, then runs `profile_parse.extract_resume_text` + `parse_resume_text` LLM-assisted. Every failure is per-job (`fail_profile_job`, terminal — a retry is a new PUT/upload from the client) while the tick itself runs inside `best_effort("profile.jobs")` so repeated DRAIN failures alert without one bad upload spamming it. 32 new tests (`test_profile_jobs.py`, `test_schedules_profile_jobs.py`, plus a DDL-mirror test in `test_db.py`) — path-traversal rejection covered from both the unit level (`_resolve_user_relative_path`) and end-to-end through `drain_once`. Full suite green, ruff clean.
| 2026-08-30 | sonnet | **xhigh /code-review of PR #241 (M4 CLI seam) found and fixed a real cross-milestone bug plus 5 hardening issues.** The most consequential: `hunter/profile_render.py::render_all`'s "full overwrite, never a merge" contract was false for files the current profile no longer needs — a track dropped from `variants` left its stale `base_cv_<track>.md` on disk, and a cleared `generation_notes` left a stale `generation_rules.local.md` that `hunter/gen_prompt.py::_local_tail()` (M2/wave-2, PR #239) keeps splicing into every future generation prompt regardless of whether the profile still wants it — reproduced live by a review sub-agent. Fixed by deleting both classes of stale file inside `render_all` itself. Also fixed in `tools/render_profile.py` (M4, PR #241): never called `profile_schema.validate()`, so a malformed/empty profile silently rendered a blank `candidate.yaml` at exit 0 (now prints a stderr `WARNING:` and still renders — this CLI transforms data, it doesn't gate content quality); `render_all(...)` had no exception handling, so an `OSError` (out_dir colliding with an existing file, a permissions error) crashed with a raw traceback instead of the documented "ERROR: ..." contract; read the input as `\"utf-8\"` instead of `\"utf-8-sig\"`, so a Windows-authored profile.json with a UTF-8 BOM failed to parse. Both `tools/parse_resume.py` and `tools/render_profile.py` now reconfigure stdout/stderr to UTF-8 (matching `tools/preview_judge.py`'s existing precedent) and pass `ensure_ascii=False` on their JSON output, matching ~25 other JSON-write call sites in the codebase — closes a live Windows `UnicodeEncodeError` risk on a non-ASCII file path in an error message. `hunter/profile_parse.py::extract_resume_text` (M3, PR #240) now checks file existence up front for every extension, since a missing `.pdf` was reported as the misleading "No extractable text" (`hunter.pdf_text`'s extractor is best-effort and swallows every exception into `\"\"`) instead of "File not found". One review finding deliberately left unfixed: a missing/invalid LLM API key still degrades `parse_resume_text` silently to a leftovers-only parse — this is M3's existing, deliberate "parse never hard-fails" contract, not something a review fixup should override. 7 new/extended tests (stale-file cleanup ×2, the validate() warning, the OSError-on-write path, UTF-8-BOM input, the missing-.pdf message, multi-leftover `--upload-id` stamping). Full suite 2982 green (was 2975), ruff clean, mypy baseline confirmed unchanged at 217. |
