...
post block). Threaded through `ScoutCandidate.permalink` → `telegram_relay.build_payload()`
→ `job.raw["permalink"]` on the bot side (the synthetic `job.url` above is untouched —
this is convenience-only, never used for dedup/fetch/routing) → an extra "🔗 Post:" line
in the pre-apply Telegram notification (`hunter/main.py::_auto_apply_all`, source-
agnostic — any Job with `raw["permalink"]` gets it) and in `notify.py`'s `--dry-run`
preview. It is NOT `manual_only` (new
