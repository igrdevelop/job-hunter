...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-07 | fable | **Shared-WAL dir mount + Sheets-garbage guard** (live incident: tracker.db corrupted — the single-FILE bind mount of tracker.db gave the bot and job-hunter-api containers each their OWN `-wal`/`-shm` sidecars over one shared main file; the copies diverged and `PRAGMA integrity_check` failed; recovered via `.recover`). Fixes: (1) `TRACKER_DB_PATH` in `hunter/config.py` is now env-overridable; `docker-compose.yml` mounts the DIRECTORY `./db` (env `TRACKER_DB_PATH=/app/db/tracker.db`) so WAL sidecars are shared with the api container — one-time host migration `mkdir -p db && mv tracker.db* db/` (api compose mirrors this in its own repo). (2) `tracker.insert_pulled_rows` now skips sheet rows whose ID is not 8 alphanumeric chars — garbage rows in the Sheet (URL/bare number in the ID column from an old column-shift incident) kept resurrecting into the DB on every start-up pull. Regression test in `tests/test_bootstrap_dedup.py`. |
...
| 2026-08-06 | sonnet | **Full-stack / tech-lead title filter — RU+EN blocking** (owner chat request, corrected mid-flight — see docs/AGENT_LOG.md for the full story). `hunter/filter_config.py`'s `exclude_levels` gained RU tech-lead substrings (`техлид`, `тех-лид`, `тех лид`, `технический лид`) alongside the existing EN entries. `hunter/filters.py`'s `_FULLSTACK_RE` extended to also match RU spellings (`фулстек`/`фуллстек`/`фулстэк`/`фуллстэк`, spaced/hyphenated) — the existing Angular+Node/Nuxt exception (owner's earlier documented preference, `test_fullstack_angular_node_kept`) is kept and applies identically to RU-titled roles, since the Angular/backend-stack checks are language-agnostic Latin-word substring matches. New RU tests in `test_filters_config_patches.py` + `test_filters_unwanted_2026_06.py`. |
| 2026-08-06 | fable | **Gmail `invalid_scope` fix** (branch claude/gmail-token-expired-f80064; live incident: `/hunt gmail` → "Gmail token expired … invalid_scope: Bad Request"). Root cause: the labeling feature (`7eca2c1`) upgraded `gmail_client.SCOPES` readonly→modify, and `get_gmail_service()` forced the new SCOPES onto the stored token — the refresh then requested a scope the prod refresh token was never granted; Google rejects that with `invalid_scope`, killing the whole Gmail source (NOT a revoked token — the alert's "revoked/expired" wording was misleading). Fix: load the token with its OWN granted scopes (`from_authorized_user_file(path)` falls back to the file's `scopes` field) so an old readonly token keeps reading alerts and only labeling degrades (best-effort 403, one WARNING with the re-auth cmd); `oauth_alert` now classifies `invalid_scope` (`_AUTH_MARKERS`) and sends scope-mismatch wording for it. New `tests/test_gmail_client.py` (mutation-verified) + 2 `test_oauth_alert.py` tests. **Ops:** owner re-runs `python tools/gmail_auth.py` + copies `gmail_token.json` to the VPS to re-enable labeling. Full detail in docs/AGENT_LOG.md. |
