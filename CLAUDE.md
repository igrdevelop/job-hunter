...
- DB row `sheets_dirty=1` → keep the DB: it holds an `/outcome` change the Sheet hasn't
  received yet (resync pushes it within 5 min). Known gap, same as the A–K push: an
  owner edit in O on a row that turns dirty for another reason before the next pull is
  overwritten by the DB value when that row resyncs
