...
| Date | Agent | Work |
|------|-------|------|
| 2026-08-16 | opus | **Night-weighted hunt schedule + 18:00–00:00 quiet hours** (owner: "сместить обработку вакансий чуть больше на ночь" → "добавим запусков ночью, а запусков с 18 по 00.00 вообще не будем делать"). **Measured on prod first, because the obvious lever was the wrong one:** `APPLY_QUEUE_ENABLED=true` is live, so a hunt only writes PENDING rows — but `apply_worker_loop` claims one within ~15 s, which means the hunt grid IS the generation grid. 87 applies / 14 days: 6.2/day avg, peak 14, ~30 min each, peaks at 10:00 and 19:00 Warsaw mirroring the old 08:00/19:00 bases, only ~21% inside 02:00–08:00. **Why dropping the 19:00 base could not do it:** `register()` computed `(base + idx*offset) % 1440`, and one cycle of 25 sources at 40 min spans 16h40m — the 08:00 and 13:00 cycles wrap into the evening on their own. **New `hunter/schedules/grid.py`** (pure ints, no PTB/pytz imports): the blackout is JUMPED OVER, not skipped — offsets accumulate through ALLOWED minutes only, so every source keeps a slot (skipping would starve whichever sources sit at those indices) and their order survives. `SCHEDULE_TIMES` (was hardcoded) is env-overridable, default `02:00,05:00,08:00,13:00`; new `SCHEDULE_BLACKOUT` default `18:00-00:00`; `RETRY_FAILED_TIMES` → `02:45,07:45` (a retry runs the same apply pipeline and 18:45 sat inside the quiet hours). A malformed or whole-day spec warns and falls back to the plain modulo grid — a typo in `.env` must never leave the bot unscheduled. Result: 75 → 100 slots/day, 02:00–08:00 share 19% → 32%, 18:00–00:00 exactly zero; no extra spend (dedup queues only new URLs, `MAX_JOBS_PER_RUN` still caps each run). 21 new tests (`tests/test_schedule_grid.py`) incl. a byte-for-byte pin of the old grid when no blackout is set; mutation-verified (ignoring the blackout fails 6 tests for the right reason). Full suite 2622 green, ruff clean. See docs/AGENT_LOG.md for the full entry. |
