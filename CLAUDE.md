...
hunter/
  ...
  profile_jobs.py            Atomic claim/finish/fail primitives over the shared
                            ...
                            actual render/parse execution — see Repository
                            Layout's schedules/ entry).
  ...
  schedules/                One file per JobQueue callback
    ...
    profile_jobs.py          scheduled_profile_jobs_drain (every 20s) — docs/
                            ...
                            value. Any failure calls `fail_profile_job()`; the
                            job is terminal, a retry is a new PUT/upload from
                            the client. The tick itself runs inside
